...
<head>
    ...
    @php
        $schemaType = match(true) {
            ...
            str_starts_with((string) $business->template_code, 'service_barber') => 'BeautySalon',
    ...
    <?php
        ...
        $services = (isset($services) && $services instanceof \Illuminate\Support\Collection)
            ? $services
            : collect($landingPage->custom_services ?? [])->map(fn (array $service): array => [
                ...$service,
        ...
        window.landingPageState = function() {
            return {
                ...
                waLink(label, isService = false) {
                    const actionText = isService ? 'saya ingin booking / reservasi layanan: ' : 'saya tertarik untuk memesan produk: ';
                    return '{{ $landingPage->getWhatsAppUrl() }}' + '&text=' + encodeURIComponent('Halo ' + @json($business->name) + ', ' + actionText + (label || ''));
                },
                modalQty: 1,
                toastMessage: null,
                toastTimeout: null,
                ...
                isCheckingOut: false,
                checkoutError: null,
                cart: @json($dbCartItems ?? null) || JSON.parse(localStorage.getItem('cooca_cart_{{ $business->id }}') || '[]'),
                @php
                ...
                get grandTotal() {
                    const fee = (this.checkoutForm.fulfillment_type === 'merchant_delivery') ? Number(this.checkoutForm.shipping_fee || 0) : 0;
                ...
                async fetchShippingQuote(preferredRuleId = null) {
                    ...
                    try {
                        ...
                        const response = await fetch('{{ route("public.storefront.shipping.calculate", $business->slug) }}', {
                            method: 'POST',
                            headers: {
                ...
                async submitCheckout() {
                    ...
                    const payload = {
                        ...
                        shipping_address: this.checkoutForm.fulfillment_type === 'merchant_delivery' ? this.checkoutForm.shipping_address : null,
                        shipping_rule_id: this.checkoutForm.fulfillment_type === 'merchant_delivery' ? (this.checkoutForm.shipping_rule_id || null) : null,
                        payment_method_id: this.checkoutForm.payment_method_id || null,
                        notes: this.checkoutForm.notes || null,
                        scheduled_date: this.checkoutForm.is_scheduled ? (this.checkoutForm.scheduled_date || null) : null,
                        scheduled_time_slot: this.checkoutForm.is_scheduled ? (this.checkoutForm.scheduled_time_slot || null) : null,
                    ...
                    try {
                        const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
                        const response = await fetch('{{ route("public.storefront.checkout", $business->slug) }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                        ...
                        if (response.ok && data.success) {
                            ...
                            if (response.status === 401) {
                                window.location.href = '{{ route("customer.auth.google") }}?redirect=' + encodeURIComponent(window.location.href);
                                return;
                            }
                ...
                async submitRequestOrder() {
                    ...
                    const payload = {
                        ...
                        shipping_address: this.requestOrderForm.fulfillment_type === 'merchant_delivery' ? this.requestOrderForm.shipping_address : null,
                    ...
                    try {
                        const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
                        const response = await fetch('{{ route("public.storefront.request_order", $business->slug) }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                        ...
                        if (response.ok && data.success) {
                            ...
                            this.requestOrderForm.error = data.message || 'Gagal mengirim permintaan pesanan khusus.';
                        }
                    } catch (e) {
                ...
                async submitReservation() {
                    ...
                    try {
                        const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
                        const response = await fetch('{{ route("public.storefront.reservation.submit", $business->slug) }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
...
<body class="{{ $landingPage->dark_mode ? 'dark bg-[#000000] text-[#FFFFFF]' : 'bg-[#F5F5F7] text-[#1D1D1F]' }} antialiased selection:bg-brand-primary selection:text-white pb-20 md:pb-0"
    ...
    <div x-show="toastMessage" x-cloak
        ...
        <div class="px-5 py-2.5 rounded-full bg-black/90 dark:bg-white/95 text-white dark:text-black shadow-2xl backdrop-blur-xl text-[13px] font-semibold flex items-center justify-center gap-2 border border-white/15 dark:border-black/15 text-center">
    ...
    <header class="sticky top-0 z-40 w-full backdrop-blur-xl {{ $landingPage->dark_mode ? 'bg-[#000000]/80 border-white/10' : 'bg-[#FFFFFF]/80 border-black/5' }} border-b transition-colors">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-4">
            ...
            <nav class="hidden md:flex items-center gap-1 text-[13px] font-medium text-black/70 dark:text-white/70">
                @if($sectionVisibility['services'] && $hasServices)
                    <a href="#layanan" class="px-3 py-1.5 rounded-full hover:bg-black/5 dark:hover:bg-white/10 hover:text-black dark:hover:text-white transition">Menu &amp; Katalog</a>
                @endif
                ...
                @if(auth('customer')->check())
                    ...
                    <div class="relative" x-data="{ userMenuOpen: false }">
                        <button type="button" @click="userMenuOpen = !userMenuOpen" @click.outside="userMenuOpen = false"
                            ...
                            <span class="w-6 h-6 rounded-full bg-[#007AFF] text-white text-[11px] font-bold flex items-center justify-center">
                        ...
                        <div x-show="userMenuOpen" x-transition.opacity
                            ...
                            <div class="px-3 py-2 border-b border-black/5 dark:border-white/10 mb-1">
                                ...
                                <p class="text-[11px] text-black/50 dark:text-white/50 truncate">{{ $navCust->phone }}</p>
                                <div class="mt-1.5 flex items-center justify-between text-[11px]">
                                    <span class="px-2 py-0.5 rounded-full bg-[#007AFF]/10 text-[#007AFF] font-bold capitalize">{{ $navCust->membership_tier ?? 'Bronze' }}</span>
                                    <span class="font-semibold text-black/70 dark:text-white/70">{{ number_format($navCust->loyalty_points ?? 0) }} Poin</span>
                                </div>
                            </div>
                            <a href="{{ route('customer.dashboard') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-[12px] text-black/80 dark:text-white/80 hover:bg-black/5 dark:hover:bg-white/10 transition font-medium">
                                <i data-lucide="layout-grid" class="w-4 h-4 text-[#007AFF]"></i>
                                <span>Dashboard Saya</span>
                            </a>
                            <a href="{{ route('customer.orders') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-[12px] text-black/80 dark:text-white/80 hover:bg-black/5 dark:hover:bg-white/10 transition font-medium">
                                <i data-lucide="shopping-bag" class="w-4 h-4 text-[#34C759]"></i>
                                <span>Riwayat Belanja</span>
                            </a>
                            <a href="{{ route('customer.cart') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-[12px] text-black/80 dark:text-white/80 hover:bg-black/5 dark:hover:bg-white/10 transition font-medium">
                                <i data-lucide="shopping-cart" class="w-4 h-4 text-[#FF9500]"></i>
                                <span>Keranjang Belanja</span>
                            </a>
                            <a href="{{ route('customer.profile') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-[12px] text-black/80 dark:text-white/80 hover:bg-black/5 dark:hover:bg-white/10 transition font-medium">
                            ...
                            <div class="border-t border-black/5 dark:border-white/10 my-1 pt-1">
                                <form method="POST" action="{{ route('customer.logout') }}">
                                    @csrf
                                    <button type="submit" class="w-full flex items-center gap-2.5 px-3 py-2 rounded-[12px] text-[#FF3B30] hover:bg-[#FF3B30]/10 transition font-medium text-left">
        ...
        <div x-show="mobileMenuOpen" x-transition.opacity
            ...
            @if(auth('customer')->check())
                ...
                @endphp
                <div class="p-3.5 rounded-[18px] bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 mb-2 space-y-2.5">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            <div class="w-9 h-9 rounded-full bg-[#007AFF] text-white text-[13px] font-bold flex items-center justify-center">
                                {{ strtoupper(mb_substr($mCust->name, 0, 1)) }}
                            </div>
                            <div>
                                <p class="font-bold text-[14px] text-black dark:text-white leading-tight">{{ $mCust->name }}</p>
                                <p class="text-[11.5px] text-black/50 dark:text-white/50">{{ $mCust->phone }}</p>
                            </div>
                        </div>
                        <span class="px-2 py-0.5 rounded-full bg-[#007AFF]/10 text-[#007AFF] text-[11px] font-bold capitalize">{{ $mCust->membership_tier ?? 'Bronze' }}</span>
                    </div>
                    <div class="grid grid-cols-3 gap-1.5 pt-1 text-center text-[12px]">
                        <a href="{{ route('customer.dashboard') }}" class="py-1.5 rounded-[10px] bg-white dark:bg-black/30 border border-black/5 dark:border-white/10 text-black/80 dark:text-white/80 font-medium">Dashboard</a>
                        <a href="{{ route('customer.orders') }}" class="py-1.5 rounded-[10px] bg-white dark:bg-black/30 border border-black/5 dark:border-white/10 text-black/80 dark:text-white/80 font-medium">Pesanan</a>
                        <form method="POST" action="{{ route('customer.logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="w-full py-1.5 rounded-[10px] bg-[#FF3B30]/10 text-[#FF3B30] font-semibold">Keluar</button>
                        </form>
                    </div>
                </div>
            @else
            ...
            @endif
            @if($storeSetting?->allow_reservation)
                <button type="button" @click="mobileMenuOpen = false; reservationModalOpen = true; reservationSuccess = false;" class="block w-full text-left py-2 text-[#34C759] font-semibold">Reservasi Meja / Booking</button>
            @endif
            <button type="button" @click="mobileMenuOpen = false; cartDrawerOpen = true;" class="block w-full text-left py-2 text-[#007AFF] font-semibold">Keranjang Belanja (<span x-text="cartCount"></span>)</button>
            @if($sectionVisibility['about'] && $hasAbout)
                            ...
                            @foreach($posProducts as $prod)
                                <div class="w-full sm:w-1/2 lg:w-1/4 shrink-0 p-2 sm:p-2.5">
                                    <button type="button" @click="openProduct(products.find(product => product.id === '{{ $prod->id }}'))"
                                        ...
                                        <div class="pt-2.5 border-t border-black/5 dark:border-white/5 flex items-center justify-between gap-1.5">
                                            <span class="font-bold text-[13.5px] sm:text-[14px] text-brand-primary tabular-nums tracking-tight">Rp {{ number_format($prod->selling_price, 0, ',', '.') }}</span>
                                            <div class="flex items-center gap-1.5">
                                                <button type="button" @click.stop="addToCart(products.find(p => p.id === '{{ $prod->id }}'), 1, false)"
                                                ...
                                                <button type="button" @click.stop="directCheckout(products.find(p => p.id === '{{ $prod->id }}'), 1)"
            ...
            <div x-show="activeModal === 'service' || activeModal === 'product'" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-4 flex flex-col justify-between">
                    ...
                    <div class="space-y-3 pt-3 border-t border-black/5 dark:border-white/5">
                        ...
                        <div class="flex items-center justify-between py-1 bg-black/[0.02] dark:bg-white/[0.03] px-3.5 py-2 rounded-[14px] border border-black/5 dark:border-white/5" x-show="activeModal === 'product' && isStorefrontEnabled">
                            <span class="text-[12.5px] font-semibold text-black/70 dark:text-white/70">Jumlah Pesanan:</span>
                            <div class="flex items-center gap-2.5 bg-white dark:bg-black/40 px-3 py-1 rounded-full border border-black/10 dark:border-white/10 shadow-xs">
                                <button type="button" @click="if (modalQty > 1) modalQty--" class="w-6 h-6 flex items-center justify-center text-black/70 dark:text-white/70 hover:text-black dark:hover:text-white font-bold text-[14px] active:scale-90 transition">&minus;</button>
                                <span class="text-[13px] font-extrabold text-black dark:text-white min-w-5 text-center tabular-nums" x-text="modalQty"></span>
                                <button type="button" @click="modalQty++" class="w-6 h-6 flex items-center justify-center text-black/70 dark:text-white/70 hover:text-black dark:hover:text-white font-bold text-[14px] active:scale-90 transition">&plus;</button>
                            </div>
                        </div>

                        {{-- Product Modal Actions --}}
                        <div class="flex flex-wrap sm:flex-nowrap items-center gap-2" x-show="activeModal === 'product'">
                            <template x-if="isStorefrontEnabled">
                                <button type="button" @click="directCheckout(activeItem, modalQty); activeModal = null;"
                            ...
                            <template x-if="isStorefrontEnabled">
                                <button type="button" @click="addToCart(activeItem, modalQty, true); activeModal = null;"
                            ...
                            <a :href="activeItem ? waLink(activeItem.title || activeItem.name, false) : '#'" target="_blank" rel="noopener"
                        ...
                        <div class="flex flex-wrap sm:flex-nowrap items-center gap-2" x-show="activeModal === 'service'">
                            <template x-if="activeItem && activeItem.raw_price && activeItem.raw_price > 0 && isStorefrontEnabled">
                                <button type="button" @click="directCheckout({ id: activeItem.id, name: activeItem.title || activeItem.name, price: activeItem.raw_price, image_url: activeItem.image_url }, 1); activeModal = null;"
                                    class="flex-1 h-10 px-4 rounded-full bg-brand-primary text-white text-[13px] font-semibold tracking-tight shadow-sm hover:opacity-90 active:scale-[0.97] transition flex items-center justify-center gap-2">
                                    <i data-lucide="zap" class="w-4 h-4"></i>
                                    <span>Pesan Online</span>
                                </button>
                            </template>
                            <button type="button" @click="activeModal = null; openReservationModal(activeItem?.title || activeItem?.name)"
                                class="h-10 px-4 rounded-full bg-[#007AFF]/10 text-[#007AFF] hover:bg-[#007AFF] hover:text-white text-[13px] font-semibold tracking-tight active:scale-[0.97] transition flex items-center justify-center gap-1.5"
                                :class="!(activeItem && activeItem.raw_price && activeItem.raw_price > 0 && isStorefrontEnabled) ? 'flex-1' : ''">
                                <i data-lucide="calendar" class="w-4 h-4"></i>
                                <span>Reservasi Jadwal</span>
                            </button>
                            <a :href="activeItem ? waLink(activeItem.title || activeItem.name, true) : '#'" target="_blank" rel="noopener"
                                class="h-10 px-3.5 rounded-full bg-[#34C759]/10 text-[#248A3D] dark:text-[#30D158] hover:bg-[#34C759]/20 text-[13px] font-semibold tracking-tight transition flex items-center justify-center gap-1.5"
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3.5 pt-1">
                    <template x-for="product in filteredProducts" :key="product.id">
                        <button type="button" @click="openProduct(product)"
                            ...
                            <div class="pt-2 mt-2 border-t border-black/5 dark:border-white/5 flex items-center justify-between gap-1">
                                <span class="text-[12.5px] font-bold text-brand-primary tabular-nums tracking-tight" x-text="'Rp ' + Number(product.price || 0).toLocaleString('id-ID')"></span>
                                <div class="flex items-center gap-1">
                                    <button type="button" @click.stop="addToCart(product, 1, false)" class="w-6 h-6 rounded-full bg-black/5 dark:bg-white/10 text-black/70 dark:text-white/70 hover:bg-brand-primary/15 hover:text-brand-primary transition flex items-center justify-center active:scale-90" title="Tambah ke Keranjang">
                                        <i data-lucide="plus" class="w-3 h-3"></i>
                                    </button>
                                    <button type="button" @click.stop="activeModal = null; directCheckout(product, 1)" class="h-6 px-2 rounded-full bg-brand-primary text-white text-[10.5px] font-semibold hover:opacity-90 transition active:scale-95 shadow-sm" title="Beli Langsung">
                                        <span>Beli</span>
                                    </button>
                                </div>
                            </div>
    ...
    <nav class="md:hidden fixed bottom-0 inset-x-0 z-50 bg-white/85 dark:bg-[#161617]/85 backdrop-blur-xl border-t border-black/5 dark:border-white/10 px-2 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))]" aria-label="Navigasi halaman">
        <div class="grid grid-cols-5 gap-1">
            <a href="#hero" class="text-center text-[10.5px] font-medium tracking-tight text-black/55 dark:text-white/55 py-1" :class="activeSection === 'hero' ? 'text-brand-primary font-semibold' : ''">
                <i data-lucide="home" class="w-4 h-4 mx-auto mb-0.5"></i>
                <span>Home</span>
            </a>
            <a href="#layanan" class="text-center text-[10.5px] font-medium tracking-tight text-black/55 dark:text-white/55 py-1" :class="activeSection === 'services' ? 'text-brand-primary font-semibold' : ''">
                <i data-lucide="package" class="w-4 h-4 mx-auto mb-0.5"></i>
                <span>Katalog</span>
            </a>
            
            {{-- Cart Tab with dynamic badge --}}
            <button type="button" @click="cartDrawerOpen = true" class="relative text-center text-[10.5px] font-medium tracking-tight text-black/55 dark:text-white/55 py-1 hover:text-brand-primary transition">
                <div class="relative w-4 h-4 mx-auto mb-0.5 flex items-center justify-center">
                    <i data-lucide="shopping-bag" class="w-4 h-4"></i>
                    <span x-show="cartCount > 0" class="absolute -top-1.5 -right-2.5 px-1 py-0.2 rounded-full text-[9px] font-bold bg-[#007AFF] text-white tabular-nums leading-none shadow-xs" x-text="cartCount"></span>
                </div>
                <span>Keranjang</span>
            </button>

            {{-- Reservation / Booking Tab --}}
            <button type="button" @click="openReservationModal()" class="text-center text-[10.5px] font-medium tracking-tight text-black/55 dark:text-white/55 py-1 hover:text-brand-primary transition">
                <i data-lucide="calendar" class="w-4 h-4 mx-auto mb-0.5"></i>
                <span>Reservasi</span>
            </button>

            <a href="#lokasi" class="text-center text-[10.5px] font-medium tracking-tight text-black/55 dark:text-white/55 py-1" :class="activeSection === 'contact' ? 'text-brand-primary font-semibold' : ''">
    ...
    <div x-show="isStorefrontEnabled && cartCount > 0" x-cloak
        ...
        <button type="button" @click="cartDrawerOpen = true"
            ...
            <div class="relative flex items-center justify-center">
                ...
                <span class="absolute -top-2 -right-2.5 px-1.5 py-0.2 rounded-full text-[10.5px] font-extrabold bg-[#007AFF] text-white tabular-nums shadow-sm"
                      x-text="cartCount"></span>
            </div>
    ...
    {{-- ========================================================================= --}}
    <div x-show="cartDrawerOpen" x-cloak class="fixed inset-0 z-[70] overflow-hidden" role="dialog" aria-modal="true">
            ...
            <div class="w-screen max-w-md bg-white dark:bg-[#1C1C1E] shadow-2xl border-l border-black/5 dark:border-white/10 flex flex-col justify-between"
                ...
                <div class="p-5 border-b border-black/5 dark:border-white/10 flex items-center justify-between">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 class="text-[16px] font-bold text-black dark:text-white tracking-tight">Keranjang Belanja</h3>
                            <span class="text-[11.5px] text-black/50 dark:text-white/50 tabular-nums"><span x-text="cartCount"></span> item dipilih</span>
                        </div>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-4 divide-y divide-black/5 dark:divide-white/10">
                    <template x-for="item in cart" :key="item.id">
                        <div class="pt-4 first:pt-0 flex items-start gap-3.5">
                            <div class="w-16 h-16 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 shrink-0 overflow-hidden flex items-center justify-center">
                                <template x-if="item.image_url">
                                    <img :src="item.image_url" :alt="item.name" class="w-full h-full object-cover">
                                </template>
                                <i x-show="!item.image_url" data-lucide="package" class="w-6 h-6 text-black/30 dark:text-white/30"></i>
                            </div>
                            <div class="flex-1 min-w-0 space-y-1">
                                <h4 class="text-[14px] font-bold text-black dark:text-white truncate" x-text="item.name"></h4>
                                <span class="text-[12.5px] font-semibold text-[#007AFF] tabular-nums block" x-text="formatPrice(item.price)"></span>

                                <div class="flex items-center justify-between pt-1">
                                    <div class="flex items-center gap-2 bg-black/5 dark:bg-white/5 px-2 py-0.5 rounded-full">
                                        <button type="button" @click="updateQuantity(item.id, -1)" class="w-5 h-5 flex items-center justify-center text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white font-bold">&minus;</button>
                                        <span class="text-[12px] font-bold text-black dark:text-white min-w-4 text-center tabular-nums" x-text="item.quantity"></span>
                                        <button type="button" @click="updateQuantity(item.id, 1)" class="w-5 h-5 flex items-center justify-center text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white font-bold">&plus;</button>
                                    </div>
                                    <button type="button" @click="removeFromCart(item.id)" class="text-black/40 hover:text-[#FF3B30] text-[11.5px] transition">Hapus</button>
                                </div>
                            </div>
                        </div>
                    </template>
                ...
                <div class="p-5 border-t border-black/5 dark:border-white/10 space-y-3 bg-black/[0.02] dark:bg-white/[0.02]">
                    <div class="flex items-center justify-between text-[14px]">
                        <span class="font-medium text-black/60 dark:text-white/60">Subtotal Belanja</span>
                        <span class="font-extrabold text-[17px] text-black dark:text-white tabular-nums" x-text="formatPrice(cartTotal)"></span>
                    </div>
        ...
        <div class="w-full max-w-lg bg-white dark:bg-[#1C1C1E] rounded-[28px] p-6 shadow-2xl border border-black/10 dark:border-white/10 max-h-[90vh] overflow-y-auto space-y-5"
            ...
            <div class="flex items-center justify-between pb-3 border-b border-black/5 dark:border-white/10">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-[17px] font-bold text-black dark:text-white tracking-tight">Checkout Pesanan</h3>
                        <span class="text-[11.5px] text-black/50 dark:text-white/50">{{ $business->name }}</span>
                    </div>
                </div>
                <button type="button" @click="checkoutModalOpen = false" class="text-black/40 hover:text-black dark:text-white/40 dark:hover:text-white">
            ...
            @guest('customer')
                <div class="p-3.5 rounded-[16px] bg-gradient-to-br from-[#007AFF]/10 to-[#5856D6]/10 border border-[#007AFF]/20 space-y-2.5 text-center sm:text-left">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                        <div class="space-y-0.5">
                            <p class="text-[13px] font-bold text-black dark:text-white">Masuk dengan Google</p>
                            <p class="text-[11.5px] text-black/60 dark:text-white/60">Wajib login &amp; verifikasi WhatsApp sebelum memesan.</p>
                        </div>
            ...
            @else
                <div class="p-3 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[12px] flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-[#34C759]"></span>
                        <span class="text-black/70 dark:text-white/70">Akun: <strong class="text-black dark:text-white">{{ auth('customer')->user()->name }}</strong></span>
                    </div>
                    @if(!auth('customer')->user()->isPhoneVerified())
                        <a href="{{ route('customer.otp') }}" class="text-[#FF9500] font-semibold hover:underline">Verifikasi OTP &rarr;</a>
                    @else
            ...
            <form @submit.prevent="submitCheckout()" class="space-y-4">
                {{-- Data Pelanggan --}}
                <div class="space-y-3">
                    <span class="text-[12px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">1. Data Pemesan</span>
                    <div>
                        <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Nama Lengkap <span class="text-red-500">*</span></label>
                        <input type="text" x-model="checkoutForm.customer_name" required placeholder="Contoh: Budi Santoso"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Nomor WhatsApp <span class="text-red-500">*</span></label>
                            <input type="tel" x-model="checkoutForm.customer_phone" required placeholder="08123456789"
                                   class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Email (Opsional)</label>
                            <input type="email" x-model="checkoutForm.customer_email" placeholder="budi@example.com"
                                   class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3 pt-2">
                    <span class="text-[12px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">2. Pengiriman</span>
                    <div class="grid grid-cols-2 gap-2">
                        <button type="button" @click="setFulfillment('pickup')"
                                :class="checkoutForm.fulfillment_type === 'pickup' ? 'border-[#007AFF] bg-[#007AFF]/10 text-[#007AFF] font-bold' : 'border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 text-black/70 dark:text-white/70'"
                        ...
                        <button type="button" @click="setFulfillment('merchant_delivery')"
                                :class="checkoutForm.fulfillment_type === 'merchant_delivery' ? 'border-[#007AFF] bg-[#007AFF]/10 text-[#007AFF] font-bold' : 'border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 text-black/70 dark:text-white/70'"
                                class="p-3 rounded-[14px] border text-left text-[12.5px] transition flex items-center gap-2">
                    ...
                    <div x-show="checkoutForm.fulfillment_type === 'merchant_delivery'" class="space-y-3 pt-1">
                        <div>
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Alamat Pengiriman Lengkap <span class="text-red-500">*</span></label>
                            <textarea x-model="checkoutForm.shipping_address" rows="2" placeholder="Nama jalan, nomor rumah, RT/RW, kelurahan, patokan..."
                        ...
                        <div x-show="checkoutForm.shipping_options && checkoutForm.shipping_options.length > 0" class="space-y-1.5">
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70">Pilihan Kurir &amp; Tarif Ongkir</label>
                            <div class="space-y-1.5">
                                <template x-for="opt in checkoutForm.shipping_options" :key="opt.id">
                                    <label class="p-2.5 rounded-[12px] border border-black/10 dark:border-white/10 flex items-center justify-between cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition"
                                           :class="checkoutForm.shipping_rule_id === opt.id ? 'border-[#007AFF] bg-[#007AFF]/5' : ''">
                                        <div class="flex items-center gap-2.5">
                                            <input type="radio" name="shipping_rule_choice" :value="opt.id"
                                                   x-model="checkoutForm.shipping_rule_id"
                                                   @change="fetchShippingQuote(opt.id)"
                                                   class="text-[#007AFF] focus:ring-[#007AFF]">
                                            <div>
                                                <span class="text-[13px] font-bold text-black dark:text-white block" x-text="opt.name"></span>
                                                <span class="text-[11px] text-black/50 dark:text-white/50" x-text="opt.description"></span>
                                            </div>
                                        </div>
                                        <div>
                                            <span x-show="opt.is_free" class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-[#34C759]/10 text-[#34C759]">Gratis</span>
                                            <span x-show="!opt.is_free" class="text-[12.5px] font-bold text-black dark:text-white tabular-nums" x-text="formatPrice(opt.fee)"></span>
                                        </div>
                                    </label>
                                </template>
                            </div>
                        ...
                        <div x-show="checkoutForm.is_loading_shipping" class="text-[11.5px] text-[#007AFF] flex items-center gap-1.5 py-1">
                ...
                @if($storeSetting?->allow_scheduled_order)
                    <div class="space-y-3 pt-2">
                        <div class="flex items-center justify-between">
                            <span class="text-[12px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">Jadwal Pesanan</span>
                            <label class="relative inline-flex items-center cursor-pointer">
                                ...
                                <span class="ml-2 text-[12px] font-medium text-black/70 dark:text-white/70">Pesan Terjadwal</span>
                            </label>
                        </div>
                        <div x-show="checkoutForm.is_scheduled" x-transition class="space-y-3 p-3 rounded-[14px] bg-black/[0.02] dark:bg-white/[0.03] border border-black/10 dark:border-white/10">
                            <div>
                                <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Pilih Tanggal <span class="text-red-500">*</span></label>
                                <input type="date" min="{{ date('Y-m-d') }}" x-model="checkoutForm.scheduled_date" :required="checkoutForm.is_scheduled"
                            ...
                            <div>
                                <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Slot Waktu (Opsional)</label>
                ...
                @if($paymentMethods->isNotEmpty())
                    <div class="space-y-3 pt-2">
                        <span class="text-[12px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">3. Pembayaran Transfer Bank / QRIS</span>
                        <div class="space-y-2">
                            @foreach($paymentMethods as $pm)
                                <label class="p-3 rounded-[14px] border border-black/10 dark:border-white/10 flex items-center justify-between cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition"
                                       :class="checkoutForm.payment_method_id === '{{ $pm->id }}' ? 'border-[#007AFF] bg-[#007AFF]/5' : ''">
                                    <div class="flex items-center gap-3">
                                        <input type="radio" name="payment_method_id" value="{{ $pm->id }}" x-model="checkoutForm.payment_method_id" class="text-[#007AFF] focus:ring-[#007AFF]">
                                        <div>
                                            <span class="text-[13.5px] font-bold text-black dark:text-white block">{{ $pm->bank_name }}</span>
                                            @if($pm->account_number)
                                                <span class="text-[11.5px] font-mono text-black/60 dark:text-white/60">{{ $pm->account_number }} a/n {{ $pm->account_holder }}</span>
                                            @else
                                                <span class="text-[11px] text-[#007AFF] font-medium">QRIS Langsung Toko</span>
                                            @endif
                                        </div>
                                    </div>
                                    <span class="text-[11px] font-bold uppercase px-2 py-0.5 rounded-full {{ $pm->type === 'qris' ? 'bg-[#5856D6]/10 text-[#5856D6]' : 'bg-black/5 dark:bg-white/10 text-black/70 dark:text-white/70' }}">
                ...
                {{-- Catatan --}}
                <div class="pt-1">
                    <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Catatan Pesanan (Opsional)</label>
                    <input type="text" x-model="checkoutForm.notes" placeholder="Contoh: Jangan terlalu pedas, titip di satpam, dll."
                           class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                </div>
                ...
                <div class="pt-4 border-t border-black/5 dark:border-white/10 space-y-2">
                    ...
                    <div x-show="checkoutForm.fulfillment_type === 'merchant_delivery'" class="flex items-center justify-between text-[13px]">
                        <span class="text-black/60 dark:text-white/60">Ongkos Kirim</span>
                        <template x-if="checkoutForm.is_free_shipping">
                            <span class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-[#34C759]/10 text-[#34C759]">Bebas Ongkir</span>
                        </template>
                        <template x-if="!checkoutForm.is_free_shipping">
                            <span class="tabular-nums font-semibold text-black dark:text-white" x-text="formatPrice(checkoutForm.shipping_fee)"></span>
                        </template>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-[15px] pt-1">
                        ...
                        <span class="text-[20px] font-extrabold text-[#007AFF] tabular-nums" x-text="formatPrice(grandTotal)"></span>
                    </div>
    ...
    {{-- ========================================================================= --}}
    <div x-show="requestOrderModalOpen" x-transition.opacity
         ...
         style="display: none;">
        <div @click.away="requestOrderModalOpen = false"
             x-show="requestOrderModalOpen"
             x-transition:enter="transition ease-out duration-300 transform"
            ...
            <div class="flex items-center justify-between pb-3 border-b border-black/5 dark:border-white/10">
                <div class="space-y-0.5">
                    <span class="text-[11px] font-bold uppercase tracking-wider text-[#007AFF] block">Request Order Khusus</span>
                    <h3 class="text-[18px] font-bold text-black dark:text-white">Pesan Kustom / Pre-Order</h3>
                </div>
                <button type="button" @click="requestOrderModalOpen = false" class="text-black/40 hover:text-black dark:text-white/40 dark:hover:text-white">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
            </div>

            <div class="p-3.5 rounded-[14px] bg-[#007AFF]/5 border border-[#007AFF]/15 text-[12px] text-black/70 dark:text-white/70 flex items-start gap-2.5">
                <i data-lucide="info" class="w-4 h-4 text-[#007AFF] shrink-0 mt-0.5"></i>
                <span>Pesanan khusus memerlukan peninjauan oleh tim kami. Anda akan menerima notifikasi penawaran harga &amp; konfirmasi ketersediaan melalui WhatsApp.</span>
            </div>
            ...
            <form @submit.prevent="submitRequestOrder()" class="space-y-4">
                {{-- Data Pemesan --}}
                <div class="space-y-3">
                    <span class="text-[11.5px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">1. Data Pemesan</span>
                    <div>
                        <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Nama Lengkap <span class="text-red-500">*</span></label>
                        <input type="text" x-model="requestOrderForm.customer_name" required placeholder="Contoh: Budi Santoso"
                               class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Nomor WhatsApp <span class="text-red-500">*</span></label>
                            <input type="tel" x-model="requestOrderForm.customer_phone" required placeholder="08123456789"
                                   class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Email (Opsional)</label>
                            <input type="email" x-model="requestOrderForm.customer_email" placeholder="budi@example.com"
                                   class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3 pt-2">
                    <span class="text-[11.5px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">2. Kebutuhan Pesanan</span>
                    <div>
                        <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Item / Menu yang Diminta <span class="text-red-500">*</span></label>
                        <input type="text" x-model="requestOrderForm.item_name" required placeholder="Contoh: Paket Katering 50 Porsi, Tumpeng Mini, Souvenir Khusus"
                               class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Perkiraan Jumlah / Porsi <span class="text-red-500">*</span></label>
                            <input type="number" min="1" x-model="requestOrderForm.item_qty" required
                                   class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Target Tanggal Dibutuhkan</label>
                            <input type="date" min="{{ date('Y-m-d') }}" x-model="requestOrderForm.scheduled_date"
                                   class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Detail &amp; Spesifikasi Kebutuhan</label>
                        <textarea x-model="requestOrderForm.notes" rows="3" placeholder="Tuliskan selengkap mungkin: jenis masakan, kemasan box/bungkus, pantangan/alergi, budget target, dll."
                                  class="w-full p-3 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF] resize-none"></textarea>
                    </div>
                </div>
                ...
                {{-- Pengiriman --}}
                <div class="space-y-3 pt-2">
                    <span class="text-[11.5px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">3. Opsi Pengambilan / Kirim</span>
                    <div class="grid grid-cols-2 gap-2">
                        <button type="button" @click="requestOrderForm.fulfillment_type = 'pickup'"
                                :class="requestOrderForm.fulfillment_type === 'pickup' ? 'border-[#007AFF] bg-[#007AFF]/10 text-[#007AFF] font-bold' : 'border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 text-black/70 dark:text-white/70'"
                                class="p-3 rounded-[14px] border text-left text-[12.5px] transition flex items-center gap-2">
                            <i data-lucide="store" class="w-4 h-4 shrink-0"></i>
                            <span>Ambil Sendiri</span>
                        </button>
                        <button type="button" @click="requestOrderForm.fulfillment_type = 'merchant_delivery'"
                                :class="requestOrderForm.fulfillment_type === 'merchant_delivery' ? 'border-[#007AFF] bg-[#007AFF]/10 text-[#007AFF] font-bold' : 'border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 text-black/70 dark:text-white/70'"
                                class="p-3 rounded-[14px] border text-left text-[12.5px] transition flex items-center gap-2">
                            <i data-lucide="truck" class="w-4 h-4 shrink-0"></i>
                    ...
                    <div x-show="requestOrderForm.fulfillment_type === 'merchant_delivery'" class="pt-1">
                        <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Alamat Tujuan Pengiriman</label>
                        <textarea x-model="requestOrderForm.shipping_address" rows="2" placeholder="Nama jalan, nomor gedung/rumah, kelurahan..."
                                  class="w-full p-3 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF] resize-none"></textarea>
                    </div>
                </div>
    ...
    <div x-show="reservationModalOpen" x-cloak
         @keydown.escape.window="reservationModalOpen = false"
         class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md"
         x-transition:enter="transition ease-out duration-200"
         x-transition:enter-start="opacity-0"
         x-transition:enter-end="opacity-100"
        ...
        <div class="w-full max-w-lg bg-white dark:bg-[#1C1C1E] rounded-[28px] p-6 shadow-2xl border border-black/10 dark:border-white/10 max-h-[90vh] overflow-y-auto space-y-5"
            ...
            <div class="flex items-center justify-between pb-3 border-b border-black/5 dark:border-white/10">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-[17px] font-bold text-black dark:text-white tracking-tight">Reservasi &amp; Booking Jadwal</h3>
                        <span class="text-[11.5px] text-black/50 dark:text-white/50">{{ $business->name }}</span>
                    </div>
                </div>
                <button type="button" @click="reservationModalOpen = false" class="text-black/40 hover:text-black dark:text-white/40 dark:hover:text-white">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
            </div>
            ...
            <template x-if="reservationSuccess">
                <div class="p-4 rounded-[16px] bg-[#34C759]/10 border border-[#34C759]/20 text-center space-y-2">
                    <div class="w-10 h-10 rounded-full bg-[#34C759]/20 text-[#34C759] flex items-center justify-center mx-auto">
                    ...
                    <p class="text-[12.5px] text-black/70 dark:text-white/70">
                        Tim kami akan segera memeriksa ketersediaan jadwal/meja dan mengirimkan konfirmasi melalui WhatsApp ke nomor Anda.
                    </p>
                    <div class="pt-2">
            ...
            <template x-if="!reservationSuccess">
                <div>
                    <template x-if="reservationError">
                        <div class="mb-4 p-3.5 rounded-[14px] bg-[#FF3B30]/10 border border-[#FF3B30]/20 text-[#FF3B30] text-[12.5px] font-medium flex items-center gap-2">
                            <i data-lucide="alert-triangle" class="w-4 h-4 shrink-0"></i>
                    ...
                    @guest('customer')
                        <div class="mb-4 p-3.5 rounded-[16px] bg-gradient-to-br from-[#007AFF]/10 to-[#5856D6]/10 border border-[#007AFF]/20 flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
                            <div class="space-y-0.5">
                                <p class="text-[13px] font-bold text-black dark:text-white">Masuk dengan Google</p>
                                <p class="text-[11.5px] text-black/60 dark:text-white/60">Wajib login &amp; verifikasi WhatsApp untuk reservasi.</p>
                            </div>
                            <a href="{{ route('customer.auth.google') }}?redirect={{ urlencode(url()->current()) }}"
                               class="px-4 py-2 bg-[#007AFF] text-white rounded-xl text-[12.5px] font-bold hover:bg-[#0062CC] transition active:scale-95 shrink-0 flex items-center gap-1.5 shadow-sm">
                                Login Google
                    ...
                    @else
                        <div class="mb-4 p-3 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[12px] flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <span class="w-2 h-2 rounded-full bg-[#34C759]"></span>
                                <span class="text-black/70 dark:text-white/70">Akun: <strong class="text-black dark:text-white">{{ auth('customer')->user()->name }}</strong></span>
                            </div>
                    ...
                    <form @submit.prevent="submitReservation()" class="space-y-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Nama Lengkap <span class="text-red-500">*</span></label>
                                <input type="text" x-model="reservationForm.customer_name" required placeholder="Contoh: Budi Santoso"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Nomor WhatsApp <span class="text-red-500">*</span></label>
                                    <input type="tel" x-model="reservationForm.customer_phone" required placeholder="08123456789"
                                           class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#34C759]">
                                </div>
                                <div>
                                    <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Email (Opsional)</label>
                                    <input type="email" x-model="reservationForm.customer_email" placeholder="budi@example.com"
                                           class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#34C759]">
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- Waktu & Jumlah Tamu --}}
                        <div class="space-y-3 pt-2">
                            <span class="text-[11.5px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45 block">2. Jadwal &amp; Tamu</span>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Tanggal Reservasi <span class="text-red-500">*</span></label>
                                    <input type="date" min="{{ date('Y-m-d') }}" x-model="reservationForm.reservation_date" required
                                           class="w-full h-11 px-3.5 rounded-[14px] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[13.5px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#34C759]">
                                </div>
                                <div>
                                    <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Waktu / Jam Kunjungan <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70 mb-1">Jumlah Orang / Tamu <span class="text-red-500">*</span></label>
                                <div class="flex items-center gap-3">
                                    <div class="flex items-center gap-3 bg-black/5 dark:bg-white/5 px-3 py-1.5 rounded-[14px] border border-black/10 dark:border-white/10">
                                        <button type="button" @click="if (reservationForm.guest_count > 1) reservationForm.guest_count--" class="w-7 h-7 flex items-center justify-center rounded-full bg-black/10 dark:bg-white/10 text-black dark:text-white font-bold">&minus;</button>
                                        <span class="text-[14px] font-extrabold text-black dark:text-white min-w-8 text-center tabular-nums" x-text="reservationForm.guest_count + ' Orang'"></span>
                                        <button type="button" @click="reservationForm.guest_count++" class="w-7 h-7 flex items-center justify-center rounded-full bg-black/10 dark:bg-white/10 text-black dark:text-white font-bold">&plus;</button>
                                    </div>
                                    <span class="text-[11.5px] text-black/50 dark:text-white/50">Dapat disesuaikan jika rombongan</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 pt-1">
                            <label class="block text-[12px] font-semibold text-black/70 dark:text-white/70">Catatan Tambahan (Layanan / Meja Khusus)</label>
                            <textarea x-model="reservationForm.notes" rows="2" placeholder="Contoh: Meja non-smoking, baby chair, dekorasi ulang tahun, dll."
                        ...
                        <div class="pt-3 border-t border-black/5 dark:border-white/10">
                            <button type="submit" :disabled="isSubmittingReservation"
                                ...
                                <i x-show="!isSubmittingReservation" data-lucide="arrow-right" class="w-4 h-4"></i>
                            </button>
                        </div>
                    </form>
                </div>
