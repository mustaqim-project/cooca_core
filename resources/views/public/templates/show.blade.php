    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-7 space-y-6">
                <div class="space-y-3">
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#34C759]/10 text-[#34C759] dark:text-[#30D158]">
                        <i data-lucide="sparkles" class="w-3.5 h-3.5"></i>
                        <span>{{ $template['category'] }} • 100% Gratis</span>
                    </div>
                    <h1 class="text-2xl sm:text-4xl font-extrabold text-[#1D1D1F] dark:text-[#F5F5F7] leading-tight tracking-tight">
                        {{ $template['name'] }}
                    </h1>
                ...
                <div class="p-5 sm:p-6 rounded-[22px] bg-black/[0.02] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.06] space-y-3.5">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-[#1D1D1F] dark:text-[#F5F5F7]">Apa yang Anda Dapatkan di Template Ini:</h3>
                ...
                <div class="p-4 sm:p-5 rounded-[20px] bg-black/[0.02] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.06] text-xs text-[#6E6E73] dark:text-[#86868B] flex items-center gap-3.5">
                    ...
                    <div>
                        <span class="font-bold text-[#1D1D1F] dark:text-[#F5F5F7] block">Aman &amp; Siap Digunakan</span>
                        <span>Kompatibel dengan Microsoft Excel 2013+, Google Sheets, dan WPS Office tanpa macro VBA berbahaya.</span>
                    </div>
                </div>
            </div>

            <!-- Right: Lead Capture Form Card (5 Cols Sticky) -->
            <div class="lg:col-span-5 bg-white dark:bg-[#1C1C1E] border border-black/[0.06] dark:border-white/[0.08] p-6 sm:p-7 rounded-[26px] shadow-sm lg:sticky lg:top-24" x-data="{
                submitted: false,
                loading: false,
                errorMessage: '',
                ...
                <div x-show="!submitted">
                    <div class="text-center mb-5 space-y-1.5">
                        <div class="w-12 h-12 rounded-[16px] bg-[#007AFF]/10 text-[#007AFF] dark:text-[#0A84FF] flex items-center justify-center mx-auto mb-2">
                            <i data-lucide="download" class="w-6 h-6"></i>
                        </div>
                        <h3 class="text-lg font-bold text-[#1D1D1F] dark:text-[#F5F5F7]">Unduh Gratis Sekarang</h3>
                        <p class="text-xs text-[#6E6E73] dark:text-[#86868B]">Masukkan kontak Anda untuk mengunduh spreadsheet resmi.</p>
                    </div>

                    <form @submit.prevent="submitLead" class="space-y-4">
                        <div x-show="errorMessage" class="p-3 rounded-[14px] bg-[#FF3B30]/10 border border-[#FF3B30]/20 text-[#FF3B30] text-xs font-medium" x-text="errorMessage"></div>

                        <div>
                            <label class="block text-xs font-semibold text-[#1D1D1F] dark:text-[#F5F5F7] mb-1.5">Nama Lengkap *</label>
                            <input type="text" x-model="form.name" required placeholder="Contoh: Budi Santoso" class="w-full h-11 px-3.5 bg-black/[0.02] dark:bg-white/[0.04] border border-black/[0.08] dark:border-white/[0.1] rounded-[14px] text-[#1D1D1F] dark:text-[#F5F5F7] text-xs sm:text-sm focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 focus:outline-none transition-all placeholder-[#6E6E73]/50">
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-[#1D1D1F] dark:text-[#F5F5F7] mb-1.5">Nomor WhatsApp / HP *</label>
                            <input type="tel" x-model="form.phone" required placeholder="Contoh: 081234567890" class="w-full h-11 px-3.5 bg-black/[0.02] dark:bg-white/[0.04] border border-black/[0.08] dark:border-white/[0.1] rounded-[14px] text-[#1D1D1F] dark:text-[#F5F5F7] text-xs sm:text-sm focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 focus:outline-none transition-all placeholder-[#6E6E73]/50">
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-[#1D1D1F] dark:text-[#F5F5F7] mb-1.5">Nama Usaha / Toko</label>
                            <input type="text" x-model="form.business_name" placeholder="Contoh: Toko Sembako Berkah" class="w-full h-11 px-3.5 bg-black/[0.02] dark:bg-white/[0.04] border border-black/[0.08] dark:border-white/[0.1] rounded-[14px] text-[#1D1D1F] dark:text-[#F5F5F7] text-xs sm:text-sm focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 focus:outline-none transition-all placeholder-[#6E6E73]/50">
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-[#1D1D1F] dark:text-[#F5F5F7] mb-1.5">Email (Opsional)</label>
                            <input type="email" x-model="form.email" placeholder="email@domain.com" class="w-full h-11 px-3.5 bg-black/[0.02] dark:bg-white/[0.04] border border-black/[0.08] dark:border-white/[0.1] rounded-[14px] text-[#1D1D1F] dark:text-[#F5F5F7] text-xs sm:text-sm focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/20 focus:outline-none transition-all placeholder-[#6E6E73]/50">
                        </div>

                        <button type="submit" :disabled="loading" class="w-full py-3.5 rounded-[14px] bg-[#007AFF] hover:bg-[#0071E3] text-white font-semibold text-xs flex items-center justify-center gap-2 shadow-sm active:scale-[0.98] transition-all">
                ...
                <div x-show="submitted" x-cloak class="text-center py-6 space-y-4">
                    <div class="w-16 h-16 rounded-full bg-[#34C759]/10 text-[#34C759] dark:text-[#30D158] flex items-center justify-center mx-auto">
                    ...
                    <div class="p-4 rounded-[18px] bg-black/[0.02] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.06] text-left space-y-2">
                    ...
                    <div class="space-y-2.5 pt-1">
                        <a :href="downloadUrl" download class="w-full py-3.5 rounded-[14px] bg-[#007AFF] hover:bg-[#0071E3] text-white font-semibold text-xs flex items-center justify-center gap-2 shadow-sm transition-all active:scale-[0.98]">
                    ...
                    <div class="pt-6 border-t border-black/[0.06] dark:border-white/[0.08] mt-4">
                        ...
                        <a href="{{ route('register') }}" class="text-xs font-bold text-[#007AFF] dark:text-[#0A84FF] hover:underline inline-flex items-center gap-1">
                            <span>Daftar Cooca UMKM Gratis Selamanya</span>
        ...
        <div class="border-t border-black/[0.06] dark:border-white/[0.08] pt-12 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <a href="{{ route('template.show', $ot['slug']) }}" class="bg-white dark:bg-[#1C1C1E] border border-black/[0.06] dark:border-white/[0.08] p-5 rounded-[22px] group flex flex-col justify-between shadow-sm hover:shadow-md hover:border-[#007AFF]/30 hover:-translate-y-0.5 transition-all">
                    <div class="space-y-2">
                        <span class="text-[10px] uppercase font-bold text-[#007AFF] dark:text-[#0A84FF]">{{ $ot['category'] }}</span>
                        <h4 class="text-sm font-bold text-[#1D1D1F] dark:text-[#F5F5F7] group-hover:text-[#007AFF] dark:group-hover:text-[#0A84FF] transition-colors leading-snug">{{ $ot['name'] }}</h4>
                    </div>
