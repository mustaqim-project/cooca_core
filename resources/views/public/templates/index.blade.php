...
@section('description', 'Koleksi template pembukuan Excel gratis untuk UMKM: Buku Kas Harian Warung, Laporan Keuangan
    Sederhana, Kartu Stok Opname, dan Invoice Profesional.')
@section('keywords', 'template pembukuan excel gratis, download format buku kas warung, template laporan laba rugi
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-14">
            ...
            <div class="text-center max-w-3xl mx-auto space-y-3">
                ...
                <p class="text-sm sm:text-base text-[#6E6E73] dark:text-[#86868B] max-w-2xl mx-auto leading-relaxed">
                    Format spreadsheet siap pakai dengan formula otomatis standar Apple HIG. Didesain khusus untuk
                    mempermudah operasional toko, warung, kafe, dan produsen rumahan tanpa rumus rumit.
                </p>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-2 gap-3 sm:gap-6 lg:gap-8">
                @foreach ($templates as $tpl)
                    @if ($loop->first)
                        ...
                        <div
                            class="col-span-2 bg-white dark:bg-[#1C1C1E] border border-black/[0.06] dark:border-white/[0.08] rounded-[24px] sm:rounded-[28px] p-5 sm:p-8 flex flex-col justify-between shadow-sm hover:shadow-md hover:border-[#007AFF]/30 transition-all group">
                            <div class="space-y-3.5 sm:space-y-4">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-8 h-8 sm:w-10 sm:h-10 rounded-[12px] bg-[#34C759]/10 text-[#34C759] dark:text-[#30D158] flex items-center justify-center">
                                ...
                                <div>
                                    <h2
                                        class="text-base sm:text-2xl font-bold text-[#1D1D1F] dark:text-[#F5F5F7] group-hover:text-[#007AFF] dark:group-hover:text-[#0A84FF] transition-colors leading-snug">
                                        {{ $tpl['name'] }}
                                    </h2>
                                ...
                                @if (!empty($tpl['highlights']))
                                    <div
                                        class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-3 border-t border-black/[0.04] dark:border-white/[0.06]">
                                        @foreach ($tpl['highlights'] as $highlight)
                                            <div
                                                class="flex items-center gap-2 text-xs text-[#1D1D1F]/80 dark:text-[#F5F5F7]/80">
                                                <i data-lucide="check-circle-2"
                                                    class="w-3.5 h-3.5 text-[#34C759] dark:text-[#30D158] shrink-0"></i>
                            ...
                            <div
                                ...
                                <a href="{{ route('template.show', $tpl['slug']) }}"
                                    class="px-5 py-2.5 rounded-[14px] bg-[#007AFF] hover:bg-[#0071E3] text-white font-semibold text-xs flex items-center gap-2 shadow-sm active:scale-95 transition-all">
                    ...
                    @else
                        ...
                        <div
                            class="col-span-1 bg-white dark:bg-[#1C1C1E] border border-black/[0.06] dark:border-white/[0.08] rounded-[20px] sm:rounded-[24px] p-4 sm:p-7 flex flex-col justify-between shadow-sm hover:shadow-md hover:border-[#007AFF]/30 transition-all group">
                            <div class="space-y-2.5 sm:space-y-4">
                                ...
                                <div>
                                    <h2
                                        class="text-xs sm:text-lg font-bold text-[#1D1D1F] dark:text-[#F5F5F7] group-hover:text-[#007AFF] dark:group-hover:text-[#0A84FF] transition-colors leading-snug line-clamp-2">
                                        {{ $tpl['name'] }}
                                    </h2>
                                    <p
                                        class="text-[11px] sm:text-xs text-[#6E6E73] dark:text-[#86868B] leading-relaxed mt-1 line-clamp-2 hidden sm:block">
                                        {{ $tpl['description'] }}
                                    </p>
                                </div>
                                ...
                                @if (!empty($tpl['highlights']))
                                    <div
                                        class="space-y-1.5 pt-2 border-t border-black/[0.04] dark:border-white/[0.06] hidden sm:block">
                                        @foreach (array_slice($tpl['highlights'], 0, 2) as $highlight)
                                            <div
                                                class="flex items-center gap-2 text-[11px] text-[#1D1D1F]/80 dark:text-[#F5F5F7]/80">
            ...
            <div
                ...
                <div class="space-y-3 max-w-xl text-center lg:text-left">
                    ...
                    <p class="text-xs sm:text-sm text-[#86868B] leading-relaxed">
                        Dengan Cooca, setiap transaksi kasir langsung menghasilkan laporan keuangan, mencatat buku kas
                        harian, dan memotong stok barang secara otomatis.
                    </p>
                </div>
                <a href="{{ route('register') }}"
                    class="px-8 py-3.5 rounded-[16px] bg-[#007AFF] hover:bg-[#0071E3] text-white font-semibold text-xs flex items-center gap-2 shrink-0 shadow-sm active:scale-95 transition-all">
