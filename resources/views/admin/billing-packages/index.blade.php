...
@section('content')
    <div class="space-y-6" x-data="{
        ...
        <!-- Page Header Bento -->
        <div class="rounded-[22px] bg-white/80 dark:bg-[#1C1C1E]/80 border border-black/[0.06] dark:border-white/[0.08] backdrop-blur-md p-6 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-bold bg-black/6 dark:bg-white/8 text-black/60 dark:text-white/60">
                    <i data-lucide="package" class="w-4 h-4 text-[#30B0C7] dark:text-[#40C8E0]" stroke-width="1.5"></i>
                    <span>{{ $packages->count() }} Paket di Katalog</span>
                </span>
            </div>
        </div>

        <!-- Apple Pill Segmented Control -->
        <div class="inline-flex flex-wrap p-1 rounded-[16px] bg-black/[0.05] dark:bg-white/[0.07] border border-black/[0.04] dark:border-white/[0.06] text-[13px] font-medium gap-1">
            @foreach ($tabMap as $tabKey => $tabItem)
                <a href="{{ route('admin.billing-packages.index', $tabKey) }}"
                    class="px-4 py-2 rounded-[12px] inline-flex items-center gap-2 {{ $type === $tabKey ? $tabItem['activeText'] : 'text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white hover:bg-black/[0.03] dark:hover:bg-white/[0.05]' }} transition-all font-semibold">
                    ...
                    @if (isset($counts[$tabKey]) && $counts[$tabKey] > 0)
                        <span class="px-2 py-0.5 rounded-full text-[11px] font-bold {{ $type === $tabKey ? 'bg-white/20 text-white' : 'bg-black/6 dark:bg-white/8 text-black/55 dark:text-white/55' }}">
                            {{ $counts[$tabKey] }}
                        </span>
                    @endif
                </a>
        ...
        @if ($type === App\Models\BillingPackage::TYPE_AI_TOKEN)
            <div class="rounded-[18px] p-4 bg-[#AF52DE]/8 border border-[#AF52DE]/20 text-[12px] text-black/65 dark:text-white/65 flex items-start gap-3 backdrop-blur-sm">
        ...
        @elseif($type === App\Models\BillingPackage::TYPE_STORAGE)
            <div class="rounded-[18px] p-4 bg-[#30B0C7]/8 border border-[#30B0C7]/20 text-[12px] text-black/65 dark:text-white/65 flex items-start gap-3 backdrop-blur-sm">
        ...
        @if ($type === App\Models\BillingPackage::TYPE_SUBSCRIPTION)
            <div class="rounded-[22px] bg-white/80 dark:bg-[#1C1C1E]/80 border border-[#5856D6]/20 backdrop-blur-md p-6 shadow-sm">
                <form method="POST" action="{{ route('admin.settings.billing') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-1">
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">Bulanan (Rp)</label>
                            <input type="number" name="subscription_price_monthly"
                                value="{{ $subscriptionPriceMonthly ?? 129000 }}" min="0" step="1000"
                                class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">Tahunan (Rp)</label>
                            <input type="number" name="subscription_price_annual"
                                value="{{ $subscriptionPriceAnnual ?? 1290000 }}" min="0" step="1000"
                                class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">Kuota Token AI / Bulan</label>
                            <input type="number" name="subscription_ai_tokens_monthly"
                                value="{{ $subscriptionAiTokensMonthly ?? 10000000 }}" min="0" step="100000"
                                class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">Badge Promo Tahunan</label>
                            <input type="text" name="subscription_annual_discount_badge"
                                value="{{ $subscriptionAnnualDiscountBadge ?? 'Hemat 2 Bulan' }}" maxlength="64"
                                class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                        </div>
                    </div>
                </form>
            </div>
        @elseif($type === App\Models\BillingPackage::TYPE_AI_TOKEN)
            <div class="rounded-[22px] bg-white/80 dark:bg-[#1C1C1E]/80 border border-[#AF52DE]/20 backdrop-blur-md p-6 shadow-sm">
                <form method="POST" action="{{ route('admin.settings.billing') }}" class="space-y-4">
                    @csrf
                    <div class="flex items-center justify-between pb-3 border-b border-black/[0.06] dark:border-white/[0.08]">
                        <div class="flex items-center gap-2.5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">Harga per Top Up (Rp)</label>
                            <input type="number" name="ai_token_topup_price" value="{{ $aiTokenTopupPrice ?? 50000 }}" min="0" step="1000"
                                class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#AF52DE]/50">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">Jumlah Token per Top Up</label>
        ...
        @elseif($type === App\Models\BillingPackage::TYPE_STORAGE)
            <div class="rounded-[22px] bg-white/80 dark:bg-[#1C1C1E]/80 border border-[#30B0C7]/20 backdrop-blur-md p-6 shadow-sm">
                <form method="POST" action="{{ route('admin.settings.billing') }}" class="space-y-4">
                    @csrf
                    <div class="flex items-center justify-between pb-3 border-b border-black/[0.06] dark:border-white/[0.08]">
                        <div class="flex items-center gap-2.5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
                        <div>
                            ...
                            <input type="number" name="storage_topup_price" value="{{ $storageTopupPrice ?? 50000 }}" min="0" step="1000"
                                class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#30B0C7]/50">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">GB per Top Up</label>
                            <input type="number" name="storage_topup_gb" value="{{ $storageTopupGb ?? 1 }}" min="1"
                                class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#30B0C7]/50">
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-black/60 dark:text-white/60 mb-1.5">Kapasitas Dasar Owner (GB)</label>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 items-start">
            ...
            <form method="POST" action="{{ route('admin.billing-packages.store') }}"
                class="rounded-[22px] bg-white/80 dark:bg-[#1C1C1E]/80 border border-black/[0.06] dark:border-white/[0.08] backdrop-blur-md p-6 space-y-4 shadow-sm lg:sticky lg:top-20">
                @csrf
                ...
                <div>
                    ...
                    <input name="name" required maxlength="120"
                        ...
                        class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price" required min="0" step="any" placeholder="0"
                        class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                    <p class="text-[11px] text-[#34C759] dark:text-[#30D158] mt-1 font-medium">💡 Set <strong>0</strong> untuk promo marketing/trial gratis (aktif instan tanpa pembayaran).</p>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" maxlength="1000" placeholder="Keterangan manfaat paket..."
                        class="w-full px-3.5 py-2.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] leading-relaxed text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50"></textarea>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" name="duration_days" required min="1" placeholder="30"
                                class="w-full h-10 px-3 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                        </div>
                        <div>
                            ...
                            <input type="number" name="token_quantity" min="1" placeholder="10000000"
                                class="w-full h-10 px-3 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                        </div>
                    </div>
                @elseif($type === App\Models\BillingPackage::TYPE_AI_TOKEN)
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" name="token_quantity" required min="1" placeholder="50000000"
                                class="w-full h-10 px-3 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#AF52DE]/50 transition">
                        </div>
                        <div>
                ...
                @else
                    <div>
                        ...
                        <input type="number" name="storage_gb" required min="0.01" step="0.01" placeholder="2"
                            class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#30B0C7]/50 transition">
                    </div>
                @endif
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" min="0" max="999" value="0"
                        class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                </div>

                <button type="submit"
                    class="w-full h-11 rounded-[14px] text-[13px] font-bold text-white bg-[#007AFF] hover:bg-[#0071E3] active:scale-[0.98] transition-all inline-flex items-center justify-center gap-2 shadow-md shadow-[#007AFF]/25">
            ...
            <div class="space-y-4">
                @forelse($packages as $package)
                    <div class="rounded-[22px] p-5 sm:p-6 border backdrop-blur-md transition-all {{ $package->is_active ? 'border-black/[0.06] dark:border-white/[0.08] bg-white/80 dark:bg-[#1C1C1E]/80 shadow-sm' : 'border-[#FF3B30]/25 bg-[#FF3B30]/5' }}">
                        <!-- Card Header -->
                        <div class="flex items-start justify-between gap-4">
                                ...
                                <div class="flex items-center gap-2.5 flex-wrap">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-[#FF3B30]/15 text-[#C41E17] dark:text-[#FF453A]">
                                            <span class="w-1.5 h-1.5 rounded-full bg-[#FF3B30]"></span> Nonaktif
                                        </span>
                                    @endif
                                </div>
                                @if ($package->description)
                                    <p class="text-[12px] text-black/50 dark:text-white/50 mt-1 leading-relaxed">{{ $package->description }}</p>
                                @endif
                            ...
                            <form method="POST" action="{{ route('admin.billing-packages.toggle', $package) }}">
                                @csrf
                                <button type="submit"
                                    class="h-8 px-3 rounded-[10px] text-[12px] font-bold transition-all inline-flex items-center gap-1.5 {{ $package->is_active ? 'bg-[#FF3B30]/10 text-[#FF3B30] dark:text-[#FF453A] hover:bg-[#FF3B30]/15' : 'bg-[#34C759]/10 text-[#34C759] dark:text-[#30D158] hover:bg-[#34C759]/15' }} active:scale-[0.98]">
                                    <i data-lucide="{{ $package->is_active ? 'pause-circle' : 'play-circle' }}" class="w-4 h-4" stroke-width="1.5"></i>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4">
                            <div class="p-3.5 rounded-[14px] bg-black/[0.03] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.04]">
                                <div class="text-[11px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45">Harga</div>
                                <div class="tabular-nums font-extrabold text-[#34C759] dark:text-[#30D158] text-[16px] mt-0.5">
                                    @if ((float) $package->price <= 0)
                                        <span class="inline-flex items-center gap-1 text-[#34C759] dark:text-[#30D158]">
                                            <span>Rp 0</span>
                                            <span class="text-[10px] px-2 py-0.5 rounded-full bg-[#34C759]/15 font-bold">PROMO GRATIS</span>
                                        </span>
                                    @else
                            ...
                            @if ($type === App\Models\BillingPackage::TYPE_SUBSCRIPTION)
                                <div class="p-3.5 rounded-[14px] bg-black/[0.03] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.04]">
                                    ...
                                    <div class="tabular-nums font-bold text-black dark:text-white text-[16px] mt-0.5">{{ $package->duration_days }} hari</div>
                                </div>
                                <div class="p-3.5 rounded-[14px] bg-black/[0.03] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.04] col-span-2 sm:col-span-1">
                                    <div class="text-[11px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45">Token AI Paket</div>
                                    <div class="tabular-nums font-bold text-[#AF52DE] dark:text-[#BF5AF2] text-[16px] mt-0.5">
                            ...
                            @elseif($type === App\Models\BillingPackage::TYPE_AI_TOKEN)
                                <div class="p-3.5 rounded-[14px] bg-black/[0.03] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.04]">
                                    ...
                                    <div class="tabular-nums font-bold text-[#AF52DE] dark:text-[#BF5AF2] text-[16px] mt-0.5">{{ number_format($package->token_quantity, 0, ',', '.') }}</div>
                                </div>
                                <div class="p-3.5 rounded-[14px] bg-black/[0.03] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.04] col-span-2 sm:col-span-1">
                                    <div class="text-[11px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45">Masa Berlaku</div>
                                    <div class="tabular-nums font-bold text-black dark:text-white text-[16px] mt-0.5">{{ $package->token_expiry_days ?? 30 }} hari</div>
                                </div>
                            @else
                                <div class="p-3.5 rounded-[14px] bg-black/[0.03] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.04]">
                                    <div class="text-[11px] font-bold uppercase tracking-wider text-black/45 dark:text-white/45">Kapasitas Storage</div>
                                    <div class="tabular-nums font-bold text-[#30B0C7] dark:text-[#40C8E0] text-[16px] mt-0.5">
                                ...
                                <div class="p-3.5 rounded-[14px] bg-black/[0.03] dark:bg-white/[0.04] border border-black/[0.04] dark:border-white/[0.04] col-span-2 sm:col-span-1">
                                    ...
                                    <div class="font-bold text-[#34C759] dark:text-[#30D158] text-[16px] mt-0.5">Permanen</div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="border-t border-black/[0.04] dark:border-white/[0.06] pt-3.5 mt-4 flex items-center justify-end gap-2">
                            <button type="button" @click="openEdit('{{ $package->id }}')"
                                class="h-8 px-3.5 rounded-[10px] text-[12px] font-bold text-[#007AFF] dark:text-[#0A84FF] bg-[#007AFF]/10 hover:bg-[#007AFF]/15 active:scale-[0.98] transition-all inline-flex items-center gap-1.5">
                ...
                @empty
                    <div class="rounded-[22px] p-12 text-center border-2 border-dashed border-black/10 dark:border-white/15 bg-black/[0.01] dark:bg-white/[0.02]">
                        <div class="w-14 h-14 mx-auto rounded-[16px] bg-black/[0.04] dark:bg-white/[0.06] flex items-center justify-center text-black/30 dark:text-white/30 mb-3">
                        ...
                        <p class="text-[12px] text-black/50 dark:text-white/50 mt-1">Gunakan formulir di samping untuk menambahkan paket pertama.</p>
                    </div>
                @endforelse
            </div>
        </div>

        <!-- Edit Package Modal (Apple Sheet) -->
        <div x-show="editOpen" x-cloak
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div @click.away="closeEdit()"
                class="rounded-[24px] bg-white dark:bg-[#1C1C1E] border border-black/10 dark:border-white/15 p-6 sm:p-7 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto shadow-2xl">
                ...
                <div class="flex items-center justify-between border-b border-black/[0.06] dark:border-white/[0.08] pb-3.5">
                    ...
                    <button type="button" @click="closeEdit()"
                        class="p-1.5 rounded-[10px] text-black/40 dark:text-white/40 hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
                ...
                <form :action="'/admin/billing-packages/' + editForm.id" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" x-model="editForm.name" required maxlength="120"
                            class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="price" x-model="editForm.price" required min="0" step="any"
                            class="w-full h-10 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[10px] text-[16px] sm:text-[13px] tabular-nums font-bold text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                        <p class="text-[11px] text-[#34C759] dark:text-[#30D158] mt-1 font-medium">Set <strong>0</strong> untuk promo trial gratis (tanpa konfirmasi pembayaran).</p>
                    </div>
                    ...
                    <div class="pt-3.5 border-t border-black/[0.06] dark:border-white/[0.08] flex items-center justify-end gap-2.5">
                        <button type="button" @click="closeEdit()"
                            class="h-10 px-4 rounded-[12px] text-[13px] font-semibold text-black/70 dark:text-white/70 bg-black/[0.05] dark:bg-white/[0.08] hover:bg-black/[0.08] transition">Batal</button>
                        <button type="submit"
                            class="h-10 px-5 rounded-[12px] text-[13px] font-bold text-white bg-[#007AFF] hover:bg-[#0071E3] active:scale-[0.98] transition-all inline-flex items-center gap-2 shadow-md shadow-[#007AFF]/25">
