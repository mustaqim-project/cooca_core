@extends('layouts.admin', [
    ...
    'headerSubtitle' => 'Tambahkan metode transfer bank atau QRIS baru untuk pembayaran langganan tenant Cooca',
])
    ...
    <div class="max-w-3xl space-y-6" x-data="{
        ...
        <form method="POST" action="{{ route('admin.payment-accounts.store') }}" enctype="multipart/form-data"
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-black/45 dark:text-white/45 mt-1">Kode unik sistem (otomatis disesuaikan tanpa spasi).</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" x-model="type" required
                            ...
                            <option value="bank_transfer" {{ old('type', 'bank_transfer') === 'bank_transfer' ? 'selected' : '' }}>
                                Transfer Bank Manual
                            </option>
                            <option value="qris" {{ old('type') === 'qris' ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="instructions" rows="3"
                        ...
                        class="w-full px-3.5 py-2.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[12px] text-[16px] md:text-[13px] leading-relaxed text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#007AFF]/30 focus:border-[#007AFF] transition">{{ old('instructions') }}</textarea>
                    <p class="text-[11px] text-black/45 dark:text-white/45 mt-1">Petunjuk ini akan muncul pada pop-up instruksi bayar yang dibaca oleh tenant.</p>
                </div>
            </div>
            ...
            <div class="space-y-4 pt-4 border-t border-black/[0.06] dark:border-white/[0.08]">
                ...
                <div>
                    <label class="block text-[13px] font-semibold text-black/80 dark:text-white/80 mb-1.5">
                        File Gambar QRIS (PNG / JPG / WEBP - Maks. 3MB)
                    </label>
                ...
                <div x-show="qrPreview" x-cloak
                    ...
                    <span class="text-[12px] font-semibold text-black/60 dark:text-white/60 block">Preview QR Code:</span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="icon"
                            class="w-full h-11 px-3.5 bg-black/[0.04] dark:bg-white/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-[12px] text-[16px] md:text-[13px] font-medium text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/30 focus:border-[#007AFF] transition">
                            <option value="credit-card" {{ old('icon') === 'credit-card' ? 'selected' : '' }}>credit-card (Transfer)</option>
                            <option value="qr-code" {{ old('icon') === 'qr-code' ? 'selected' : '' }}>qr-code (QRIS)</option>
                            <option value="wallet" {{ old('icon') === 'wallet' ? 'selected' : '' }}>wallet (Dompet)</option>
                            <option value="landmark" {{ old('icon') === 'landmark' ? 'selected' : '' }}>landmark (Bank)</option>
                            <option value="building-2" {{ old('icon') === 'building-2' ? 'selected' : '' }}>building-2 (Perusahaan)</option>
                        </select>
                    </div>
