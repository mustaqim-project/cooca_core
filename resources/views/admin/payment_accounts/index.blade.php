...
@section('content')
    <div class="space-y-6" x-data="{
            ...
            <form method="GET" action="{{ route('admin.payment-accounts.index') }}"
                ...
                @if ($search !== '' || $type !== '')
                    <a href="{{ route('admin.payment-accounts.index') }}"
                        class="h-8 px-2.5 rounded-[8px] text-[12px] font-medium text-[#FF3B30] dark:text-[#FF453A] hover:bg-[#FF3B30]/8 transition-colors inline-flex items-center gap-1 self-end sm:self-auto"><i
                            data-lucide="x" class="w-3.5 h-3.5" stroke-width="1.5"></i>Reset</a>
                @endif
            </form>
            <a href="{{ route('admin.payment-accounts.create') }}"
                class="h-9 px-4 rounded-[10px] text-[13px] font-semibold text-white bg-[#007AFF] hover:bg-[#0071E3] active:scale-[0.97] active:opacity-80 transition-all flex items-center justify-center gap-1.5 shadow-[0_1px_2px_rgba(0,122,255,0.25)] shrink-0"><i
                    data-lucide="plus-circle" class="w-4 h-4" stroke-width="1.5"></i><span>Tambah Rekening Baru</span></a>
        </div>
                ...
                <table class="w-full text-left text-[13px]">
                    <thead>
                        <tr class="border-b border-black/5 dark:border-white/10">
                            ...
                            <th class="px-4 py-2.5 text-[11px] font-semibold text-black/40 dark:text-white/40">Bank /
                                Saluran Pembayaran</th>
                            <th class="px-4 py-2.5 text-[11px] font-semibold text-black/40 dark:text-white/40">Nomor
                                Rekening / Kode</th>
                            <th class="px-4 py-2.5 text-[11px] font-semibold text-black/40 dark:text-white/40">Atas Nama
                                (A/N)</th>
                            <th class="px-4 py-2.5 text-[11px] font-semibold text-black/40 dark:text-white/40">Tipe</th>
                            <th class="px-4 py-2.5 text-[11px] font-semibold text-black/40 dark:text-white/40 text-center">
                                Status</th>
                    ...
                    <tbody class="divide-y divide-black/[0.04] dark:divide-white/[0.06]">
                        @forelse($accounts as $account)
                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                <td class="px-4 py-3 font-mono font-bold text-black/50 dark:text-white/50">
                                    #{{ $account->sort_order }}</td>
                                <td class="px-4 py-3">
                                    ...
                                    @else bg-[#AF52DE]/10 text-[#AF52DE] dark:text-[#BF5AF2] @endif">
                                            <i data-lucide="{{ $account->icon ?: 'credit-card' }}" class="w-4 h-4"
                                                stroke-width="1.5"></i>
                                        </div>
                                        <div>
                                            <div class="font-semibold text-black dark:text-white text-[13px]">
                                                {{ $account->bank_name }}</div>
                                            <div class="text-[11px] text-black/45 dark:text-white/45 font-mono uppercase">
                                                Code: {{ $account->bank_code }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="space-y-1">
                                        <span
                                            class="font-mono font-semibold text-black dark:text-white text-[13px] tabular-nums">{{ $account->account_number }}</span>
                                        @if ($account->qr_image_path)
                                            <div>
                                                <button type="button"
                                                    @click="openQris('{{ $account->qr_image_url }}', '{{ $account->bank_name }}')"
                                                    class="inline-flex items-center gap-1 text-[11px] font-medium text-[#34C759] dark:text-[#30D158] hover:underline"><i
                                                        data-lucide="qr-code" class="w-3 h-3"
                                                        stroke-width="1.5"></i><span>Lihat QR Code</span></button>
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3 font-medium text-black/80 dark:text-white/80">
                                    {{ $account->account_name }}</td>
                                <td class="px-4 py-3">
                                    @if ($account->type === \App\Models\PaymentAccount::TYPE_QRIS)
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#30B0C7]/12 text-[#1F7A89] dark:text-[#40C8E0]"><span
                                                class="w-1.5 h-1.5 rounded-full bg-[#30B0C7]"></span>QRIS Instant</span>
                                    @elseif($account->type === \App\Models\PaymentAccount::TYPE_E_WALLET)
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#AF52DE]/12 text-[#7C3AA6] dark:text-[#BF5AF2]"><span
                                                class="w-1.5 h-1.5 rounded-full bg-[#AF52DE]"></span>e-Wallet</span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#007AFF]/12 text-[#0062D1] dark:text-[#0A84FF]"><span
                                                class="w-1.5 h-1.5 rounded-full bg-[#007AFF]"></span>Transfer Bank</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <form method="POST"
                                        action="{{ route('admin.payment-accounts.toggle-status', $account) }}">
                                        @csrf
                                        <button type="submit"
                                            ...
                                            <span
                                                class="w-2 h-2 rounded-full {{ $account->is_active ? 'bg-[#34C759]' : 'bg-black/40 dark:bg-white/40' }}"></span>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('admin.payment-accounts.edit', $account) }}"
                                            title="Edit Rekening"
                                            class="h-7 w-7 rounded-[6px] bg-black/[0.04] dark:bg-white/[0.06] text-black/60 dark:text-white/60 hover:bg-black/[0.08] dark:hover:bg-white/[0.1] transition-colors inline-flex items-center justify-center"><i
                                                data-lucide="edit-3" class="w-4 h-4" stroke-width="1.5"></i></a>
                                        <form method="POST"
                                            ...
                                            <button type="submit" title="Hapus Rekening"
                                                class="h-7 w-7 rounded-[6px] bg-[#FF3B30]/10 text-[#FF3B30] dark:text-[#FF453A] hover:bg-[#FF3B30]/15 transition-colors inline-flex items-center justify-center"><i
                                                    data-lucide="trash-2" class="w-4 h-4"
                                                    stroke-width="1.5"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-12 text-center">
                                    <i data-lucide="wallet" class="w-12 h-12 mx-auto text-black/20 dark:text-white/20"
                                        stroke-width="1.5"></i>
                                    @if ($search !== '' || $type !== '')
                                        <p class="text-[15px] font-semibold text-black dark:text-white mt-3">Tidak
                                            ditemukan rekening</p>
                                        <p class="text-[13px] text-black/50 dark:text-white/50 mt-1">Coba ubah kata kunci
                                            pencarian atau pilih tipe lain.</p>
                                    @else
                                        <p class="text-[15px] font-semibold text-black dark:text-white mt-3">Belum ada
                                            rekening pembayaran</p>
                                        <p class="text-[13px] text-black/50 dark:text-white/50 mt-1">Klik tombol "Tambah
                                            Rekening Baru" untuk membuat metode pembayaran pertama.</p>
                                    @endif
                                </td>
                            </tr>
                        @endforelse
        ...
        <div x-show="qrisModalUrl" x-cloak
            ...
            <div
                class="sheet-material rounded-[20px] p-6 border border-black/5 dark:border-white/10 max-w-sm w-full space-y-4 text-center shadow-[0_20px_50px_rgba(0,0,0,0.25)]">
                ...
                <div class="flex justify-center">
                    <a :href="qrisModalUrl" target="_blank"
                        class="text-[13px] font-medium text-[#007AFF] dark:text-[#0A84FF] hover:underline inline-flex items-center gap-1"><i
                            data-lucide="external-link" class="w-3.5 h-3.5" stroke-width="1.5"></i><span>Buka Ukuran
                            Penuh</span></a>
                </div>
            </div>
        </div>
