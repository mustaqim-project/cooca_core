    ...
    <div class="space-y-6 pb-12" x-data="{
        ...
        <div
            ...
            <div class="space-y-1.5 max-w-3xl">
                ...
                <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                    ...
                    kolaborasi strategis sesama pemilik usaha.
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        <span class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Kuota
                            Storage</span>
                        <div
                ...
                <div
                    ...
                    <span
                        class="font-bold text-slate-700 dark:text-slate-300 font-mono">{{ number_format($storageSummary['used_mb'] ?? 0, 1) }}
                        MB / {{ $storageSummary['limit_gb'] ?? 1 }} GB</span>
                </div>
            </div>
            ...
            <div
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        <span class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Grup
                            Diskusi</span>
                        <div
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div
                    class="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200/90 dark:border-slate-800/90 shadow-xs p-4 sm:p-5 space-y-3">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-start gap-3">
                            <div
                                class="w-9 h-9 rounded-xl bg-amber-100 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 text-amber-700 dark:text-amber-400 flex items-center justify-center shrink-0 mt-0.5 shadow-2xs">
                                <i data-lucide="hard-drive" class="w-4 h-4"></i>
                            </div>
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    <h3 class="text-xs sm:text-sm font-bold text-slate-900 dark:text-white">Kuota Storage
                                        Pemilik Bisnis</h3>
                                    <span
                                        class="rounded-full px-2 py-0.5 text-[10px] font-mono font-bold uppercase {{ ($storageSummary['percentage'] ?? 0) > 80 ? 'bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-400 border-rose-200/90 dark:border-rose-800/90' : 'bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 border-amber-200/90 dark:border-amber-800/90' }}">
                    ...
                    <div class="space-y-1.5 pt-1">
                        <div class="flex items-center justify-between text-xs font-mono">
                            <span class="text-slate-500 dark:text-slate-400 text-[11px]">Terpakai: <strong
                                    class="text-slate-900 dark:text-white font-bold">{{ number_format($storageSummary['used_mb'] ?? 0, 1, ',', '.') }}
                                    MB</strong></span>
                            <span class="text-slate-500 dark:text-slate-400 text-[11px]">Batas Maks: <strong
                                    class="text-slate-800 dark:text-slate-200 font-bold">{{ number_format($storageSummary['limit_gb'] ?? 0, 2, ',', '.') }}
                                    GB</strong></span>
                        </div>
                        <div
                            class="w-full h-2 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800/80 p-0.5">
                ...
                <div id="composer-section"
                    ...
                    <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-3">
                        <div class="flex items-center gap-2.5">
                            <div
                                class="w-8 h-8 rounded-xl bg-emerald-600 text-white font-black text-xs flex items-center justify-center shadow-2xs">
                                {{ strtoupper(substr(auth()->user()->name ?? 'O', 0, 1)) }}
                            </div>
                            <div>
                                <span
                                    class="text-xs font-bold text-slate-900 dark:text-white block">{{ auth()->user()->name ?? 'Owner' }}</span>
                                <span class="text-[10px] text-emerald-600 dark:text-emerald-400 font-mono">Posting sebagai
                                    Pemilik Bisnis</span>
                            </div>
                        </div>
                    ...
                    <form method="POST" action="{{ route('community.store') }}" enctype="multipart/form-data"
                        @submit="isSubmitting = true" class="space-y-3.5">
                        @csrf

                        <!-- Content Textarea -->
                        <div class="relative">
                            <textarea name="content" required rows="3" maxlength="5000" @input="onInput($event)"
                        ...
                        <div x-show="previewUrl" x-cloak
                            class="relative rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700/80 bg-black/5 dark:bg-black/40">
                        ...
                        <div class="flex flex-wrap items-center gap-1.5 pt-1">
                            <span
                                class="text-[10px] font-mono text-slate-400 dark:text-slate-500 font-bold uppercase mr-1">Topik
                                Cepat:</span>
                            <button type="button" @click="appendTag('#TipsBisnis')"
                                class="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 text-[11px] font-medium border border-slate-200 dark:border-slate-800 transition cursor-pointer">💡
                                #TipsBisnis</button>
                            <button type="button" @click="appendTag('#ManajemenPOS')"
                                class="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 text-[11px] font-medium border border-slate-200 dark:border-slate-800 transition cursor-pointer">📊
                                #ManajemenPOS</button>
                            <button type="button" @click="appendTag('#Kolaborasi')"
                                class="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 text-[11px] font-medium border border-slate-200 dark:border-slate-800 transition cursor-pointer">🤝
                                #Kolaborasi</button>
                            <button type="button" @click="appendTag('#TanyaOwner')"
                                class="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 text-[11px] font-medium border border-slate-200 dark:border-slate-800 transition cursor-pointer">💬
                                #TanyaOwner</button>
                        </div>
                        ...
                        <!-- Actions Bottom Bar -->
                        <div
                            class="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/80 gap-3">
                            <label
                                class="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white cursor-pointer transition shadow-2xs">
                ...
                <div class="flex items-center justify-between pt-2">
                    <div class="flex items-center gap-2">
                        <h2 class="text-sm sm:text-base font-black text-slate-900 dark:text-white">Diskusi &amp; Kabar
                            Terbaru</h2>
                        <span
                            class="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[11px] font-mono font-bold">{{ $posts->total() }}</span>
                    </div>
                </div>
                ...
                <div class="space-y-5">
                    @forelse($posts as $post)
                        <article
                            class="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200/90 dark:border-slate-800/90 shadow-xs p-5 sm:p-6 space-y-4 transition hover:border-slate-300 dark:hover:border-slate-700/90"
                            id="{{ $post->id }}" x-data="postCard('{{ $post->id }}', {{ $post->liked_by_me ? 'true' : 'false' }}, {{ $post->likes_count }}, {{ $post->comments_count }})">

                            <!-- Post Author Header -->
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex items-start gap-3 min-w-0">
                                    <div
                                        class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center font-black text-sm text-emerald-700 dark:text-emerald-400 shrink-0 shadow-2xs">
                                        {{ strtoupper(substr($post->owner->name ?? 'O', 0, 1)) }}
                                    </div>
                                    <div class="min-w-0 space-y-0.5">
                                        <div class="flex items-center gap-2 flex-wrap">
                                            <span
                                                class="font-bold text-slate-900 dark:text-white text-sm truncate">{{ $post->owner->name ?? 'Owner' }}</span>
                                            <span
                                                class="rounded-full px-2 py-0.5 text-[9px] font-bold border inline-flex items-center gap-1 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 border-emerald-200/90 dark:border-emerald-800/90 uppercase tracking-wider font-mono">
                                                Owner
                                            </span>
                                        </div>
                                        <div
                                            class="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-2 flex-wrap font-sans">
                                            @if ($post->business)
                                                <span
                                                    class="inline-flex items-center gap-1 font-medium text-slate-700 dark:text-slate-300">
                                                    <i data-lucide="building-2"
                                                        class="w-3 h-3 text-cyan-600 dark:text-cyan-400"></i>
                                                    <span>{{ $post->business->name }}</span>
                                                </span>
                                                <span class="text-slate-300 dark:text-slate-600">•</span>
                                            @endif
                                            <span
                                                class="font-mono text-slate-400 dark:text-slate-500">{{ $post->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-1.5 shrink-0">
                                    <button type="button" @click="copyLink('{{ $post->id }}')"
                                        ...
                                        <i data-lucide="share-2" class="w-4 h-4"
                                            x-show="copiedPostId !== '{{ $post->id }}'"></i>
                                        <i data-lucide="check" class="w-4 h-4 text-emerald-600 dark:text-emerald-400"
                                            x-show="copiedPostId === '{{ $post->id }}'" x-cloak></i>
                                    </button>
                            ...
                            <div class="space-y-3">
                                <p
                                    class="text-xs sm:text-sm text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-line selection:bg-emerald-500 selection:text-white font-sans">
                                    {{ $post->content }}</p>

                                <!-- Attached Post Image -->
                                @if ($post->image_url)
                                    <div
                                        class="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 mt-3">
                            ...
                            <div
                                class="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-3">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <button type="button" @click="toggleLike()"
                                        ...
                                        :class="liked ?
                                            ...
                                            'text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-slate-100 dark:hover:bg-slate-800/80 border border-transparent'">
                                        <i data-lucide="heart" class="w-4 h-4 transition-transform active:scale-125"
                                            :class="liked ?
                                                'fill-rose-500 text-rose-500 dark:fill-rose-400 dark:text-rose-400' : ''"></i>
                                    ...
                                    <button type="button" @click="commentsOpen = !commentsOpen"
                                        ...
                                        :class="commentsOpen ?
                                            ...
                                            'text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-800/80 border border-transparent'">
                                        <i data-lucide="message-circle" class="w-4 h-4"></i>
                                        <span x-text="commentsCount + ' Komentar'"></span>
                                    </button>
                                </div>

                                <span class="text-[10px] font-mono text-slate-400 dark:text-slate-500">ID:
                                    {{ substr($post->id, 0, 8) }}</span>
                            </div>

                            <!-- Reactive Comments Drawer -->
                            <div x-show="commentsOpen" x-cloak
                                class="p-4 sm:p-5 rounded-xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800/80 space-y-4 transition-all">
                                ...
                                <div class="space-y-3">
                                    <template x-for="c in clientComments" :key="c.id">
                                        <div class="flex items-start gap-3">
                                            <div
                                                class="w-7 h-7 rounded-lg bg-emerald-100 dark:bg-slate-800 border border-emerald-200 dark:border-slate-700 text-emerald-700 dark:text-emerald-400 font-bold text-xs flex items-center justify-center shrink-0">
                                                <span x-text="c.user.name.charAt(0).toUpperCase()"></span>
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <div
                                                    class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-xl p-3 shadow-2xs">
                                                    <div class="flex items-center justify-between gap-2">
                                                        <span class="text-xs font-bold text-slate-900 dark:text-white"
                                                            x-text="c.user.name"></span>
                                                        <span
                                                            class="text-[10px] font-mono text-slate-400 dark:text-slate-500"
                                                            x-text="c.created_at"></span>
                                                    </div>
                                        ...
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div
                                                    ...
                                                    <div class="flex items-center justify-between gap-2">
                                                        <span
                                                            class="text-xs font-bold text-slate-900 dark:text-white">{{ $comment->user->name ?? 'Owner' }}</span>
                                                        <span
                                                            class="text-[10px] font-mono text-slate-400 dark:text-slate-500">{{ $comment->created_at->diffForHumans() }}</span>
                                                    </div>
                                                    <p
                                                        class="text-xs text-slate-700 dark:text-slate-300 mt-1 leading-relaxed whitespace-pre-line">
                                                        {{ $comment->content }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @empty
                                        <div x-show="clientComments.length === 0"
                                            class="text-center py-3 text-xs text-slate-400 dark:text-slate-500 italic">
                                ...
                                <form class="flex items-center gap-2 pt-2 border-t border-slate-200 dark:border-slate-850"
                    ...
                    @empty
                        <!-- Empty State -->
                        <div
                            class="p-12 sm:p-16 rounded-2xl bg-white dark:bg-slate-900/80 border border-dashed border-slate-200 dark:border-slate-800 text-center space-y-4">
                            <div
                                class="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-slate-400 flex items-center justify-center mx-auto shadow-inner">
                                <i data-lucide="messages-square" class="w-8 h-8"></i>
                            </div>
                            <div class="space-y-1 max-w-sm mx-auto">
                                <h4 class="text-sm sm:text-base font-black text-slate-900 dark:text-white">Belum Ada
                                    Postingan Komunitas</h4>
                                <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                                    Jadilah yang pertama berbagi cerita bisnis, tips penjualan, atau kabar pembukaan outlet
                                    baru Anda kepada sesama pengusaha.
                                </p>
                            </div>
                        </div>
                    @endforelse
                ...
                @if ($posts->hasPages())
                    <div
                        class="p-4 rounded-xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800">
                        {{ $posts->links() }}
                    </div>
                @endif
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div
                    ...
                    <div class="flex items-center gap-2.5 border-b border-slate-100 dark:border-slate-800 pb-3">
                        <div
                            class="w-8 h-8 rounded-xl bg-cyan-100 dark:bg-cyan-500/15 border border-cyan-200 dark:border-cyan-500/30 text-cyan-600 dark:text-cyan-400 flex items-center justify-center">
                            <i data-lucide="shield-check" class="w-4 h-4"></i>
                        </div>
                        <h3 id="guidelines-heading"
                            class="text-xs sm:text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">
                            Etika Komunitas Cooca</h3>
                    </div>
                ...
                <div
                    ...
                    <div class="flex items-center gap-2.5 border-b border-slate-100 dark:border-slate-800 pb-3">
                        <div
                            class="w-8 h-8 rounded-xl bg-amber-100 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 text-amber-600 dark:text-amber-400 flex items-center justify-center">
                            <i data-lucide="sparkles" class="w-4 h-4"></i>
                        </div>
                        <h3 id="topics-heading"
                            class="text-xs sm:text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">
                            Inspirasi Topik Hari Ini</h3>
                    </div>

                    <div class="space-y-2 text-xs">
                        <div
                            class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 space-y-1">
                            <span
                                class="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 font-bold uppercase block">Efisiensi
                                Biaya</span>
                            <p class="text-slate-700 dark:text-slate-300">"Bagaimana cara Anda menekan food waste atau
                                barang rusak di gudang?"</p>
                        </div>

                        <div
                            class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 space-y-1">
                            <span
                                class="text-[10px] font-mono text-cyan-600 dark:text-cyan-400 font-bold uppercase block">Strategi
                                Kasir POS</span>
                            <p class="text-slate-700 dark:text-slate-300">"Promo bundling apa yang paling ampuh menaikkan
                                rata-rata belanja kasir?"</p>
                        </div>

                        <div
                            class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 space-y-1">
                            <span
                                class="text-[10px] font-mono text-amber-600 dark:text-amber-400 font-bold uppercase block">Motivasi
                                Tim</span>
                            <p class="text-slate-700 dark:text-slate-300">"Tips menjaga semangat tim kasir dan staf dapur
                                saat jam sibuk (rush hour)."</p>
                        </div>
                    </div>
                </div>
                ...
                <div
                    class="rounded-2xl p-5 bg-emerald-50/80 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/60 shadow-xs space-y-3.5">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-500/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center">
                            <i data-lucide="message-circle" class="w-5 h-5"></i>
                        </div>
                        <div>
                            <h4 class="text-sm font-black text-slate-900 dark:text-white">Grup WhatsApp VIP Owner</h4>
                            <span class="text-[10px] text-emerald-700 dark:text-emerald-400 font-mono">Khusus Pengguna
                                Terverifikasi</span>
                        </div>
                    </div>
                    <p class="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                        Terhubung langsung secara real-time dengan ratusan pemilik bisnis, diskusi santai, dan update rilis
                        fitur eksklusif dari tim developer Cooca.
                    </p>
    ...
    <script>
        function postCard(postId, initialLiked, initialLikesCount, initialCommentsCount) {
            return {
                ...
                commentsOpen: false,
                clientComments: [],
                csrfToken: document.querySelector('meta[name="csrf-token"]') ? document.querySelector(
