    ...
    <div class="max-w-[1360px] mx-auto space-y-6 pb-12" x-data="{
        ...
        <div class="overflow-x-auto pb-1 scrollbar-none">
            <div
                ...
                <a href="{{ route('suppliers.index') }}"
                    ...
                    <span
                        class="px-1.5 py-0.2 rounded-full text-[11px] tabular-nums font-semibold bg-[#007AFF]/12 text-[#007AFF]">{{ $suppliers->total() }}</span>
                </a>
                ...
                <a href="{{ route('purchasing.bills.index') }}"
                    ...
                    <svg class="w-4 h-4 text-black/40 dark:text-white/40" fill="none" stroke="currentColor"
                        stroke-width="1.8" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6H2.25m0 0v1.5c0 .621.504 1.125 1.125 1.125H16.5m-14.25 0h14.25m0 0a3.75 3.75 0 0 0 3.75-3.75V4.5m-3.75 4.125V18.75" />
                    </svg>
        ...
        <header
            ...
            <div>
                ...
                <h1 class="text-[20px] font-semibold text-black dark:text-white tracking-tight">Direktori Pemasok &amp;
                    Vendor</h1>
                <p class="text-[13px] text-black/50 dark:text-white/50">Kelola mitra vendor bahan baku, kontak PIC, dan
                    pengadaan bisnis</p>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div
                ...
                <div class="mt-2 flex items-baseline justify-between">
                    <span
                        class="text-[24px] font-bold tabular-nums text-black dark:text-white">{{ $suppliers->total() }}</span>
            ...
            <div
                ...
                <div class="mt-2 flex items-baseline justify-between">
                    <span
                        class="text-[24px] font-bold tabular-nums text-[#007AFF] dark:text-[#0A84FF]">{{ $suppliedVendorsCount }}</span>
            ...
            <div
                ...
                <div class="mt-2 flex items-baseline justify-between">
                    <span
                        class="text-[24px] font-bold tabular-nums text-[#FF9500] dark:text-[#FF9F0A]">{{ $activeMaterialsCount }}</span>
        ...
        <div
            ...
            <form method="GET" action="{{ route('suppliers.index') }}"
                class="flex-1 flex items-center gap-2 max-w-lg">
            ...
            <div class="text-[13px] text-black/50 dark:text-white/50 font-normal self-start sm:self-auto">
                Menampilkan <span
                    class="font-medium text-black dark:text-white tabular-nums">{{ $suppliers->count() }}</span> dari
                <span class="font-medium text-black dark:text-white tabular-nums">{{ $suppliers->total() }}</span> pemasok
            </div>
        </div>
        ...
        {{-- ===================================================== --}}
        <div
            class="hidden sm:block rounded-[14px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
                        ...
                        @forelse($suppliers as $supplier)
                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @if ($supplier->notes)
                                        <div class="text-[11px] text-black/45 dark:text-white/45 truncate max-w-xs mt-0.5"
                                    ...
                                    @if ($supplier->phone)
                                        ...
                                        <div class="flex items-center gap-2">
                                            <a href="tel:{{ $supplier->phone }}"
                                                class="hover:text-[#007AFF] transition inline-flex items-center gap-1 text-black/80 dark:text-white/80 font-medium">
                                ...
                                <td class="px-4 py-3">
                                    @if ($supplier->email)
                                        <a href="mailto:{{ $supplier->email }}"
                                            class="text-[#007AFF] hover:underline inline-flex items-center gap-1 text-[12px]">
                                ...
                                {{-- Address --}}
                                <td class="px-4 py-3 text-black/50 dark:text-white/50 max-w-xs truncate"
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold tabular-nums {{ $supplier->materials_count > 0 ? 'bg-[#007AFF]/12 text-[#007AFF]' : 'bg-black/[0.06] dark:bg-white/[0.08] text-black/55 dark:text-white/55' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="py-12 text-center text-black/50 dark:text-white/50">
                                    <div
                                        class="w-12 h-12 rounded-full bg-black/[0.04] dark:bg-white/[0.06] flex items-center justify-center mx-auto mb-2.5 text-black/30 dark:text-white/30">
                                    ...
                                    <h4 class="text-[15px] font-semibold text-black dark:text-white">Belum Ada Data Pemasok
                                    </h4>
                                    <p class="text-[12px] text-black/45 dark:text-white/45 mt-1 max-w-sm mx-auto">
                                        Tambahkan pemasok untuk mengelola kontak vendor, riwayat PO, dan pengadaan bahan
                                        baku bisnis.
                                    </p>
                                    @if (\App\Support\Context::hasPermission('master_data.suppliers.manage'))
                                        <button type="button" @click="showAddModal = true"
        ...
        <div class="sm:hidden space-y-3">
            <div class="flex items-center justify-between px-1">
                <h2 class="text-[14px] font-semibold text-black dark:text-white">Daftar Pemasok</h2>
                <span class="text-[12px] text-black/45 dark:text-white/45 tabular-nums">{{ $suppliers->total() }}
                    Pemasok</span>
            </div>

            <div
                class="rounded-[14px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 overflow-hidden divide-y divide-black/[0.04] dark:divide-white/[0.06] shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
                @forelse($suppliers as $supplier)
                    @php
                        $waNumber = null;
                        if ($supplier->phone) {
                    ...
                    <div class="p-3.5 space-y-2.5 active:bg-black/[0.02] dark:active:bg-white/[0.03] transition-colors">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <h3 class="text-[14px] font-semibold text-black dark:text-white">{{ $supplier->name }}
                                </h3>
                                @if ($supplier->contact_person)
                                    <p class="text-[12px] text-black/60 dark:text-white/60 mt-0.5">PIC: <span
                                            class="font-medium text-black/80 dark:text-white/80">{{ $supplier->contact_person }}</span>
                                    </p>
                                @endif
                            </div>
                            <span
                                class="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold tabular-nums {{ $supplier->materials_count > 0 ? 'bg-[#007AFF]/12 text-[#007AFF]' : 'bg-black/[0.06] dark:bg-white/[0.08] text-black/55 dark:text-white/55' }}">
                                {{ $supplier->materials_count }} item
                            </span>
                        </div>
                        ...
                        @if ($supplier->notes)
                            <p
                                class="text-[11px] text-black/50 dark:text-white/50 bg-black/[0.02] dark:bg-white/[0.02] p-2 rounded-[8px] leading-relaxed">
                                {{ $supplier->notes }}
                            </p>
                        @endif
                        ...
                        <div class="space-y-1.5 text-[12px]">
                            @if ($supplier->phone)
                                <div class="flex items-center justify-between gap-2 pt-0.5">
                                    <a href="tel:{{ $supplier->phone }}"
                                        class="text-black/70 dark:text-white/70 font-mono tabular-nums flex items-center gap-1 hover:text-[#007AFF]">
                                        <svg class="w-3.5 h-3.5 text-black/40 dark:text-white/40" fill="none"
                                    ...
                                    @if ($waNumber)
                                        <a href="https://wa.me/{{ $waNumber }}" target="_blank"
                                            rel="noopener noreferrer"
                                            class="h-7 px-2.5 rounded-[7px] bg-[#34C759]/12 text-[#248A3D] dark:text-[#30D158] hover:bg-[#34C759]/20 transition flex items-center gap-1 text-[11px] font-semibold">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor"
                                                stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a.75.75 0 0 1-.774-.75 6.002 6.002 0 0 1 1.09-3.268C4.542 15.65 4 13.916 4 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                            ...
                            @if ($supplier->email)
                                <div class="flex items-center gap-1.5 text-black/60 dark:text-white/60">
                                    <svg class="w-3.5 h-3.5 text-black/40 dark:text-white/40" fill="none"
                                        stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <a href="mailto:{{ $supplier->email }}"
                                        class="text-[#007AFF] hover:underline truncate">{{ $supplier->email }}</a>
                                </div>
                            @endif

                            @if ($supplier->address)
                                <div class="flex items-start gap-1.5 text-black/50 dark:text-white/50">
                                    <svg class="w-3.5 h-3.5 text-black/40 dark:text-white/40 shrink-0 mt-0.5"
                        ...
                        @if (\App\Support\Context::hasPermission('master_data.suppliers.manage'))
                            <div
                                class="flex items-center justify-end gap-2 pt-2 border-t border-black/[0.04] dark:border-white/[0.06]">
                                <button type="button"
                                    @click="openEditModal({
                        id: '{{ $supplier->id }}',
                        name: '{{ addslashes($supplier->name) }}',
                        ...
                        notes: '{{ addslashes($supplier->notes ?? '') }}'
                                    ...
                                    class="h-8 px-3 rounded-[8px] text-[12px] font-medium text-[#007AFF] bg-[#007AFF]/10 hover:bg-[#007AFF]/15 transition flex items-center gap-1">
                                ...
                                <button type="button"
                                    @click="openDelete({{ $supplier->id }}, '{{ addslashes($supplier->name) }}')"
                                    class="h-8 px-3 rounded-[8px] text-[12px] font-medium text-[#FF3B30] bg-[#FF3B30]/10 hover:bg-[#FF3B30]/15 transition flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.5"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @empty
                    <div class="py-10 px-4 text-center text-black/50 dark:text-white/50">
                        ...
                        <h4 class="text-[14px] font-semibold text-black dark:text-white">Belum Ada Data Pemasok</h4>
                        <p class="text-[12px] text-black/45 dark:text-white/45 mt-1 max-w-xs mx-auto">
            ...
            @if ($suppliers->hasPages())
                <div
                    class="p-3 bg-white dark:bg-[#1C1C1E] rounded-[14px] border border-black/5 dark:border-white/5 flex items-center justify-between text-[12px]">
                    {{ $suppliers->links() }}
                </div>
            @endif
        </div>
            ...
            <div class="w-full max-w-lg rounded-[18px] bg-white/95 dark:bg-[#2C2C2E]/95 backdrop-blur-xl border border-black/5 dark:border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.25)] p-6 space-y-5 max-h-[90vh] overflow-y-auto"
                ...
                <div class="flex items-center justify-between border-b border-black/5 dark:border-white/10 pb-3.5">
                    <div>
                        ...
                        <p class="text-[13px] text-black/50 dark:text-white/50">Daftarkan mitra vendor penyedia bahan baku
                            bisnis</p>
                    </div>
                ...
                <div class="flex items-center justify-between border-b border-black/5 dark:border-white/10 pb-3.5">
                    <div>
                        ...
                        <p class="text-[13px] text-black/50 dark:text-white/50"
                            x-text="'Memperbarui: ' + editSupplier.name"></p>
                    </div>
            ...
            <div class="w-[290px] rounded-[14px] bg-white/95 dark:bg-[#2C2C2E]/95 backdrop-blur-xl overflow-hidden text-center shadow-[0_20px_50px_rgba(0,0,0,0.25)] border border-black/5 dark:border-white/10"
                ...
                <div class="px-4 pt-5 pb-4">
                    ...
                    <p class="text-[13px] text-black/60 dark:text-white/60 mt-1 leading-snug">
                        ...
                        dihapus dari sistem. Pastikan tidak ada PO aktif terkait.
                    </p>
                </div>
