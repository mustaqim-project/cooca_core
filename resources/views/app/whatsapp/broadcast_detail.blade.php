...
@section('content')
    <div class="max-w-[1360px] mx-auto space-y-6 pb-12">
        ...
        <!-- ===================================================== -->
        <header
            class="rounded-[16px] backdrop-blur-md bg-white/80 dark:bg-[#1C1C1E]/80 border border-black/5 dark:border-white/10 p-5 sm:p-6 flex items-center justify-between gap-4 flex-wrap transition-colors shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2.5 flex-wrap">
                        <h1 class="text-[20px] font-bold text-black dark:text-white tracking-tight">{{ $campaign->title }}
                        </h1>
                        @php
                    ...
                    <p class="text-[12px] text-black/50 dark:text-white/50 mt-0.5">
                        Dibuat pada {{ $campaign->created_at->format('d M Y, H:i') }} WIB &bull; Target: <span
                            class="capitalize font-semibold text-black/70 dark:text-white/70">{{ $campaign->target_filter }}</span>
                    </p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div
                        class="w-7 h-7 rounded-[8px] bg-[#5856D6]/10 text-[#5856D6] dark:text-[#5E5CE6] flex items-center justify-center">
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div
                        class="w-7 h-7 rounded-[8px] bg-[#34C759]/12 text-[#34C759] dark:text-[#30D158] flex items-center justify-center">
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div
                        class="w-7 h-7 rounded-[8px] bg-[#FF3B30]/12 text-[#FF3B30] dark:text-[#FF453A] flex items-center justify-center">
                        <i data-lucide="x-circle" class="w-3.5 h-3.5"></i>
                    </div>
                </div>
                <div
                    class="text-[20px] sm:text-[26px] font-bold tabular-nums text-[#FF3B30] dark:text-[#FF453A] tracking-tight">
        ...
        <div
            ...
            @if ($campaign->media_url)
                <div
                    class="p-2 bg-black/[0.02] dark:bg-white/[0.03] rounded-[12px] border border-black/5 dark:border-white/10 max-w-sm">
                    <img src="{{ $campaign->media_url }}" alt="Banner Promosi"
                        class="w-full h-auto rounded-[8px] object-cover">
                </div>
            @endif

            <div
                class="p-4 rounded-[12px] bg-black/[0.02] dark:bg-white/[0.03] border border-black/5 dark:border-white/10 font-mono text-[12px] text-black/90 dark:text-white/90 whitespace-pre-wrap leading-relaxed">
                {{ $campaign->message }}
            </div>
        </div>
        ...
        <div
            ...
            <div class="p-4 sm:p-5 border-b border-black/5 dark:border-white/10 flex items-center justify-between">
                ...
                <span class="text-[12px] text-black/40 dark:text-white/40 font-medium tabular-nums">Halaman
                    {{ $recipients->currentPage() }} dari {{ $recipients->lastPage() }}</span>
            </div>
