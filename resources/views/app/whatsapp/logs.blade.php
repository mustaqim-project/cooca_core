...
@section('content')
    <div class="max-w-[1360px] mx-auto space-y-6 pb-12" x-data="{ filterType: 'all' }">
        ...
        <header
            ...
            <div>
                <h1 class="text-[20px] sm:text-[24px] font-bold text-black dark:text-white tracking-tight">Log Komunikasi
                    WhatsApp</h1>
                <p class="text-[13px] text-black/50 dark:text-white/50 mt-0.5">Audit trail seluruh pesan otomatis, struk
                    kasir POS, dan blast promosi terkirim</p>
            </div>
        ...
        <div
            ...
            <div class="p-4 sm:p-5 border-b border-black/5 dark:border-white/10 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[12px] text-black/50 dark:text-white/50">Daftar transaksi pesan WhatsApp bot yang dikirim
                        ke pelanggan</p>
                </div>
                <span class="text-[12px] text-black/40 dark:text-white/40 font-medium tabular-nums">Halaman
                    {{ $logs->currentPage() }} dari {{ $logs->lastPage() }}</span>
            </div>
                ...
                <div class="text-center py-16 px-4 space-y-3">
                    ...
                    <div>
                        ...
                        <p class="text-black/50 dark:text-white/50 text-[13px] mt-1 max-w-sm mx-auto leading-relaxed">
                            Log pengiriman pesan akan otomatis tercatat setiap kali kasir mengirim struk POS atau
                            menjalankan blast promosi.
                        </p>
                    </div>
                    <a href="{{ route('whatsapp.index') }}"
                        ...
                        <tbody class="divide-y divide-black/[0.04] dark:divide-white/[0.06]">
                            @foreach ($logs as $log)
                                <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors"
                                    ...
                                    <td class="px-5 py-3">
                                        <div class="font-semibold text-black dark:text-white">{{ $log->recipient_name }}
                                        </div>
                                        <div class="text-black/50 dark:text-white/50 tabular-nums text-[12px]">
                                            {{ $log->recipient_phone }}</div>
                                    </td>
                                    <td class="px-4 py-3">
                                        @php
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold {{ $typeBadge }}">
                                            {{ $typeLabel }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 max-w-xs">
                                        <div class="text-black/80 dark:text-white/80 truncate text-[12px]"
                                            title="{{ $log->message }}">
                                    ...
                                    <td
                                        class="px-5 py-3 text-black/50 dark:text-white/50 tabular-nums text-[12px] whitespace-nowrap">
                ...
                <div class="sm:hidden divide-y divide-black/[0.04] dark:divide-white/[0.06]">
                    @foreach ($logs as $log)
                        ...
                        <div x-show="filterType === 'all' || filterType === '{{ $log->type }}'"
                            class="p-3.5 space-y-2 active:bg-black/[0.02] dark:active:bg-white/[0.03] transition-colors">
                            <div class="flex items-start justify-between gap-2">
                                <div>
                                    <div class="font-semibold text-[14px] text-black dark:text-white">
                                        {{ $log->recipient_name }}</div>
                                    <div class="text-black/50 dark:text-white/50 font-mono tabular-nums text-[11px] mt-0.5">
                                        {{ $log->recipient_phone }}</div>
                                </div>
                                <span
                                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $typeBadge }} shrink-0">
                                    {{ $typeLabel }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-center justify-between text-[12px] pt-0.5">
                                <div>
                                    @if ($log->status === 'sent')
                                        <span
                                ...
                                <span class="text-black/45 dark:text-white/45 text-[11px] tabular-nums">
                                    {{ $log->created_at->format('d/m/Y H:i') }}
                                </span>
                            </div>
                        </div>
