@extends('layouts.app', ['title' => 'Aturan Ongkir & Kurir Toko - Cooca UMKM'])
    ...
    <div class="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <div
                    ...
                    <span>/</span>
                    <span class="text-black/80 dark:text-white/80">Ongkir & Pengiriman</span>
                </div>
                <h1 class="text-2xl font-bold tracking-tight text-black dark:text-white flex items-center gap-2.5">
                    <i data-lucide="truck" class="w-6 h-6 text-[#007AFF]"></i>
                    <span>Aturan Ongkir & Kurir Toko</span>
                </h1>
                <p class="text-sm text-black/60 dark:text-white/60 mt-1">
                    Atur tarif ongkos kirim mandiri, pembagian zona radius jarak (KM), dan promo bebas ongkir otomatis untuk
                    pelanggan etalase online Anda.
                </p>
            </div>
            <div class="flex items-center gap-2.5">
                <a href="{{ route('storefront.settings.index') }}"
                    class="h-10 px-4 rounded-xl bg-black/[0.04] dark:bg-white/[0.06] hover:bg-black/[0.08] dark:hover:bg-white/[0.1] text-xs font-semibold text-black/80 dark:text-white/80 transition-all flex items-center gap-2">
                    <i data-lucide="settings" class="w-4 h-4"></i>
                    <span>Pengaturan Toko</span>
                </a>
                <button type="button" @click="openAdd()"
                    class="h-10 px-5 rounded-xl bg-[#007AFF] hover:bg-[#0071E3] text-white text-xs font-bold tracking-wide transition-all shadow-sm hover:shadow-md flex items-center gap-2">
        ...
        <div
            ...
            <div class="p-5 border-b border-black/5 dark:border-white/10 flex items-center justify-between">
                ...
                <span
                    class="text-xs font-mono px-2.5 py-1 rounded-lg bg-black/[0.03] dark:bg-white/[0.05] text-black/60 dark:text-white/60">
                    {{ $rules->count() }} Data
                </span>
            </div>
                                ...
                                <tr class="hover:bg-black/[0.01] dark:hover:bg-white/[0.01] transition-colors">
                                    ...
                                    <td class="py-3.5 px-5 text-black/60 dark:text-white/60">
                                        @if ($rule->rule_type === 'distance_tier')
                                            Radius {{ $rule->min_distance_km ?? 0 }} - {{ $rule->max_distance_km ?? '∞' }}
                                            KM
                                        @elseif ($rule->rule_type === 'free_threshold')
                                            Min. Belanja Rp
                                            {{ number_format((float) $rule->min_order_for_free, 0, ',', '.') }}
                                        @else
                                            Berlaku untuk semua jarak antar
                                        @endif
                                    </td>
                                    <td class="py-3.5 px-5 text-right font-mono font-bold text-black dark:text-white">
                                        @if ($rule->rate_amount <= 0 && $rule->rule_type === 'free_threshold')
                                            <span class="text-emerald-600 dark:text-emerald-400 font-bold">GRATIS (Rp
                                                0)</span>
                                        @else
        ...
        <div x-show="showAddModal" x-cloak
            ...
            <div class="bg-white dark:bg-[#1C1C1E] border border-black/10 dark:border-white/10 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-6"
                ...
                <form
                    ...
                    <div>
                        ...
                        <input type="text" name="name" x-model="editRule.name" required
                            ...
                            class="w-full h-11 px-4 rounded-xl bg-black/[0.02] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 text-xs font-medium text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                    </div>
