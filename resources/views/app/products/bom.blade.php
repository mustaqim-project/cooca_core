...
@section('content')
    <div class="max-w-[1360px] mx-auto space-y-6 pb-12" x-data="{
        ...
        <header
            ...
            <div>
                ...
                <div class="flex items-center gap-2.5">
                    <h1 class="text-[20px] font-semibold text-black dark:text-white tracking-tight">{{ $product->name }}
                    </h1>
                    <span
                        class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-[#34C759]/12 text-[#248A3D] dark:text-[#30D158] tabular-nums">
                        <span class="w-1.5 h-1.5 rounded-full bg-[#34C759]"></span>
                        Output: {{ $bomHeader->output_quantity }} {{ $product->outputUnit?->name ?? 'pcs' }}
                    </span>
                </div>
                <p class="text-[13px] text-black/50 dark:text-white/50 mt-0.5">Model HPP: {{ $costModel->name }}
                    ({{ strtoupper($costModel->method) }})</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            <div
                class="rounded-[14px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 p-4 flex flex-col justify-between shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
                ...
                <div class="mt-2 flex items-baseline justify-between">
                    <span class="text-[24px] font-bold tabular-nums text-[#34C759] dark:text-[#30D158]">
                        ...
                        {{ number_format((float) ($explosion['total_rolled_up_material_cost'] ?? 0), 0, ',', '.') }}
                    </span>
        ...
        <div
            ...
            <div
                class="px-4 py-3 border-b border-black/5 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02] flex items-center justify-between">
                <h2 class="text-[13px] font-semibold text-black dark:text-white">Komponen Bahan Terdaftar
                    ({{ $bomHeader->items->count() }} Komponen)</h2>
            </div>
                    ...
                    <thead>
                        <tr class="border-b border-black/5 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02]">
                            <th
                                ...
                                Komponen Bahan Baku</th>
                            <th
                                class="py-2.5 px-4 text-[11px] font-semibold uppercase tracking-wide text-black/40 dark:text-white/40 text-center">
                                Jumlah Resep</th>
                            <th
                                class="py-2.5 px-4 text-[11px] font-semibold uppercase tracking-wide text-black/40 dark:text-white/40 text-right">
                                Harga Efektif Bahan</th>
                            <th
                                class="py-2.5 px-4 text-[11px] font-semibold uppercase tracking-wide text-black/40 dark:text-white/40 text-center">
                                Susut (Waste %)</th>
                            <th
                            ...
                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                ...
                                <td class="py-3 px-4 text-center">
                                    @if ($item->waste_percentage > 0)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#FF3B30]/12 text-[#C41E17] dark:text-[#FF453A] tabular-nums">
                                ...
                                <td class="py-3 px-4 text-right tabular-nums font-bold text-[#34C759] dark:text-[#30D158]">
                                    {{ $business->currency_symbol }}
                                    {{ number_format((float) $itemSubtotal, 0, ',', '.') }}
                                </td>
        ...
        @if (\App\Support\Context::hasPermission('costing.manage'))
            <div x-show="showAddItemModal" x-cloak
                ...
                <div class="w-full max-w-md rounded-[16px] bg-white/95 dark:bg-[#2C2C2E]/95 backdrop-blur-xl border border-black/10 dark:border-white/10 p-6 space-y-4 shadow-[0_20px_50px_rgba(0,0,0,0.25)]"
                    ...
                    <div class="flex items-center justify-between border-b border-black/10 dark:border-white/10 pb-3">
                        ...
                        <button @click="showAddItemModal = false"
                            class="p-1 rounded-[6px] text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white">✕</button>
                    </div>
                    ...
                    <form method="POST" action="{{ route('bom.items.store', $bomHeader->id) }}"
                        ...
                        <div>
                            <label class="block font-medium text-black/70 dark:text-white/70 mb-1">Pilih Bahan Baku <span
                                    class="text-[#FF3B30]">*</span></label>
                            <select name="material_id" required
                                class="w-full h-10 bg-black/[0.04] dark:bg-white/[0.06] border-none rounded-[10px] px-3 text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                                <option value="">-- Pilih Bahan Baku --</option>
                                @foreach ($materials as $m)
                                    <option value="{{ $m->id }}">
                                        {{ $m->name }} ({{ $m->unit?->code }} - {{ $business->currency_symbol }}
                                        {{ number_format((float) ($m->prices->first()?->effective_cost ?? 0), 0, ',', '.') }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block font-medium text-black/70 dark:text-white/70 mb-1">Jumlah Pemakaian
                            ...
                            <div>
                                <label class="block font-medium text-black/70 dark:text-white/70 mb-1">Satuan Takar <span
                                        class="text-[#FF3B30]">*</span></label>
                                <select name="unit_id" required
                                    class="w-full h-10 bg-black/[0.04] dark:bg-white/[0.06] border-none rounded-[10px] px-3 text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                                    @foreach ($units as $u)
                                        <option value="{{ $u->id }}">{{ $u->name }} ({{ $u->code }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div>
                            <label class="block font-medium text-black/70 dark:text-white/70 mb-1">Susut Proses / Waste
                                Tambahan (%)</label>
                            <input type="number" name="waste_percentage" value="0" min="0" max="100"
                                step="0.5"
                                class="w-full h-10 bg-black/[0.04] dark:bg-white/[0.06] border-none rounded-[10px] px-3.5 text-black dark:text-white tabular-nums focus:outline-none focus:ring-2 focus:ring-[#007AFF]/50 transition">
                            <p class="text-[11px] text-black/40 dark:text-white/40 mt-1">Misal sisa adonan pada wadah atau
                                potongan terbuang.</p>
                        </div>

                        <div>
                            <label class="block font-medium text-black/70 dark:text-white/70 mb-1">Catatan Khusus
                                (Opsional)</label>
                        ...
                        <div class="pt-3 flex justify-end gap-2 border-t border-black/10 dark:border-white/10">
                            <button type="button" @click="showAddItemModal = false"
                                class="h-9 px-3.5 rounded-[10px] text-[13px] font-medium text-black/80 dark:text-white/80 bg-black/[0.06] dark:bg-white/[0.08] hover:bg-black/[0.09] transition">Batal</button>
                            <button type="submit"
                                class="h-9 px-4 rounded-[10px] text-[13px] font-semibold text-white bg-[#007AFF] hover:bg-[#0071E3] active:scale-[0.97] transition shadow-[0_1px_2px_rgba(0,122,255,0.25)]">Tambah
                                ke Resep</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="w-[290px] rounded-[14px] bg-white/95 dark:bg-[#2C2C2E]/95 backdrop-blur-xl overflow-hidden text-center shadow-[0_20px_50px_rgba(0,0,0,0.25)] border border-black/5 dark:border-white/10"
                    ...
                    <div class="px-4 pt-5 pb-4">
                        ...
                        <p class="text-[13px] text-black/60 dark:text-white/60 mt-1 leading-snug">
                            ...
                            dihapus dari formula BOM produk ini.
                        </p>
                    </div>
                    <div class="grid grid-cols-2 border-t border-black/10 dark:border-white/10 text-[15px] font-medium">
                        <button type="button" @click="closeDelete()"
                            class="py-3 text-[#007AFF] border-r border-black/10 dark:border-white/10 active:bg-black/5 transition-colors">
                            Batal
                        </button>
                        <button type="button" @click="submitDelete()"
                            class="py-3 text-[#FF3B30] font-semibold active:bg-black/5 transition-colors">
                            Hapus
                        </button>
                    </div>
                </div>
