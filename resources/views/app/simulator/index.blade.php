...
@section('content')
    <div class="max-w-[1360px] mx-auto space-y-6 pb-12" x-data="{
        ...
        {{-- ===================================================== --}}
        <header
            class="rounded-[14px] backdrop-blur-md bg-white/75 dark:bg-[#1C1C1E]/75 border border-black/5 dark:border-white/10 px-4 sm:px-6 py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                {{-- Breadcrumb minimal --}}
            ...
            @if ($products->isNotEmpty())
                <form method="GET" action="{{ route('simulator.index') }}"
                        ...
                        <select name="cost_model_id" onchange="this.form.submit()"
                            ...
                            @foreach ($products as $p)
                                @foreach ($p->costModels as $cm)
                                    <option value="{{ $cm->id }}"
                                        {{ $selectedCostModel?->id === $cm->id ? 'selected' : '' }}>
                                        {{ $p->name }} - {{ $cm->name }}
                                    </option>
                                @endforeach
                            @endforeach
        ...
        @if (!$selectedCostModel)
            ...
            {{-- ===================================================== --}}
            <div
                class="p-12 rounded-[14px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 text-center">
                <div
                    class="w-14 h-14 rounded-full bg-black/[0.04] dark:bg-white/[0.06] flex items-center justify-center mx-auto mb-3 text-black/30 dark:text-white/30">
        ...
        @else
            ...
            {{-- ===================================================== --}}
            <div
                class="rounded-[14px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 p-3 sm:p-4 space-y-2.5">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div
                    class="lg:col-span-4 rounded-[14px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 p-4 sm:p-5 space-y-5">
                    <div class="flex items-center justify-between border-b border-black/5 dark:border-white/10 pb-3">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-[#007AFF]" fill="none" stroke="currentColor" stroke-width="2"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M10.5 6h9.75M10.5 6a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-9.75 0h9.75" />
                            </svg>
                        ...
                        <div class="flex justify-between items-center text-[13px]">
                            ...
                            <span class="font-mono tabular-nums font-semibold text-[12px] px-2 py-0.5 rounded-full"
                                :class="matChange > 0 ? 'bg-[#FF3B30]/12 text-[#C41E17] dark:text-[#FF453A]' : (matChange < 0 ?
                                    ...
                                    'bg-black/[0.06] text-black/60 dark:bg-white/[0.08] dark:text-white/60')"
                                x-text="(matChange >= 0 ? '+' : '') + matChange + '%'"></span>
                        </div>
                        <input type="range" x-model.number="matChange" @input="runSimulation()" min="-50"
                            max="100" step="5"
                    ...
                    <div class="space-y-1.5 pt-2 border-t border-black/5 dark:border-white/5">
                        ...
                        <input type="range" x-model.number="labChange" @input="runSimulation()" min="-50"
                            max="100" step="5"
                            class="w-full h-1.5 bg-black/[0.08] dark:bg-white/[0.1] rounded-lg appearance-none cursor-pointer accent-[#007AFF]">
                        <div
                            class="flex justify-between text-[10px] text-black/40 dark:text-white/40 font-mono tabular-nums">
                    ...
                    <div class="space-y-1.5 pt-2 border-t border-black/5 dark:border-white/5">
                        ...
                        <input type="range" x-model.number="macChange" @input="runSimulation()" min="-50"
                            max="100" step="5"
                            class="w-full h-1.5 bg-black/[0.08] dark:bg-white/[0.1] rounded-lg appearance-none cursor-pointer accent-[#007AFF]">
                        <div
                            class="flex justify-between text-[10px] text-black/40 dark:text-white/40 font-mono tabular-nums">
                    ...
                    <div class="space-y-1.5 pt-2 border-t border-black/5 dark:border-white/5">
                        ...
                        <input type="range" x-model.number="ovhChange" @input="runSimulation()" min="-50"
                            max="100" step="5"
                            class="w-full h-1.5 bg-black/[0.08] dark:bg-white/[0.1] rounded-lg appearance-none cursor-pointer accent-[#007AFF]">
                        <div
                            class="flex justify-between text-[10px] text-black/40 dark:text-white/40 font-mono tabular-nums">
                    ...
                    <div class="space-y-1.5 pt-3 border-t border-black/10 dark:border-white/10">
                        ...
                        <input type="range" x-model.number="markupPct" @input="runSimulation()" min="10"
                            max="150" step="5"
                            class="w-full h-1.5 bg-black/[0.08] dark:bg-white/[0.1] rounded-lg appearance-none cursor-pointer accent-[#007AFF]">
                        <div
                            class="flex justify-between text-[10px] text-black/40 dark:text-white/40 font-mono tabular-nums">
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <template x-if="simResult">
                        <div class="space-y-6">
                                ...
                                <div
                                    ...
                                    <div
                                        ...
                                        {{ $business->currency_symbol }} <span
                                            x-text="Math.round(simResult.baseline.total_hpp).toLocaleString('id-ID')"></span>
                                    </div>
                                    <div
                                ...
                                <div
                                    ...
                                    <div class="text-[20px] sm:text-[22px] font-bold tabular-nums font-mono"
                                        :class="simResult.impact.delta_hpp_amount > 0 ? 'text-[#FF3B30] dark:text-[#FF453A]' : (
                                            simResult.impact.delta_hpp_amount < 0 ?
                                            'text-[#34C759] dark:text-[#30D158]' : 'text-black dark:text-white')">
                                        <span
                                            x-text="(simResult.impact.delta_hpp_amount > 0 ? '+' : '') + '{{ $business->currency_symbol }} ' + Math.round(simResult.impact.delta_hpp_amount).toLocaleString('id-ID')"></span>
                                    </div>
                                    <div class="mt-2 pt-2 border-t border-black/5 dark:border-white/5 text-[12px] font-medium flex items-center justify-between"
                                        :class="simResult.impact.delta_hpp_percentage > 0 ?
                                        ...
                                        <tbody class="divide-y divide-black/[0.04] dark:divide-white/[0.06]">
                                            ...
                                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                                ...
                                                <td
                                                    ...
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.baseline.material_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td
                                                    class="px-4 py-3 text-right font-mono tabular-nums font-semibold text-black dark:text-white">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.simulated.material_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td class="px-4 py-3 text-right font-mono tabular-nums font-semibold"
                                                    :class="matChange > 0 ? 'text-[#FF3B30] dark:text-[#FF453A]' : (matChange <
                                                        0 ? 'text-[#34C759] dark:text-[#30D158]' : 'text-black/40')">
                                            ...
                                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                                ...
                                                <td
                                                    class="px-4 py-3 text-right font-mono tabular-nums text-black/60 dark:text-white/60">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.baseline.labor_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td
                                                    class="px-4 py-3 text-right font-mono tabular-nums font-semibold text-black dark:text-white">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.simulated.labor_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td class="px-4 py-3 text-right font-mono tabular-nums font-semibold"
                                                    :class="labChange > 0 ? 'text-[#FF3B30] dark:text-[#FF453A]' : (labChange <
                                                        0 ? 'text-[#34C759] dark:text-[#30D158]' : 'text-black/40')">
                                            ...
                                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                                ...
                                                <td
                                                    class="px-4 py-3 text-right font-mono tabular-nums text-black/60 dark:text-white/60">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.baseline.machine_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td
                                                    class="px-4 py-3 text-right font-mono tabular-nums font-semibold text-black dark:text-white">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.simulated.machine_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td class="px-4 py-3 text-right font-mono tabular-nums font-semibold"
                                                    :class="macChange > 0 ? 'text-[#FF3B30] dark:text-[#FF453A]' : (macChange <
                                                        0 ? 'text-[#34C759] dark:text-[#30D158]' : 'text-black/40')">
                                            ...
                                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                                ...
                                                <td
                                                    class="px-4 py-3 text-right font-mono tabular-nums text-black/60 dark:text-white/60">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.baseline.overhead_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td
                                                    class="px-4 py-3 text-right font-mono tabular-nums font-semibold text-black dark:text-white">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.simulated.overhead_cost).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td class="px-4 py-3 text-right font-mono tabular-nums font-semibold"
                                                    :class="ovhChange > 0 ? 'text-[#FF3B30] dark:text-[#FF453A]' : (ovhChange <
                                                        0 ? 'text-[#34C759] dark:text-[#30D158]' : 'text-black/40')">
                                            ...
                                            <tr
                                                ...
                                                <td
                                                    class="px-4 py-3.5 text-right font-mono tabular-nums text-[#007AFF] font-bold text-[14px]">
                                                    {{ $business->currency_symbol }} <span
                                                        x-text="Math.round(simResult.simulated.total_hpp).toLocaleString('id-ID')"></span>
                                                </td>
                                                <td class="px-4 py-3.5 text-right font-mono tabular-nums font-bold"
                                                    :class="simResult.impact.delta_hpp_percentage > 0 ?
                                                        'text-[#FF3B30] dark:text-[#FF453A]' : (simResult.impact
                                                            .delta_hpp_percentage < 0 ?
                                                            'text-[#34C759] dark:text-[#30D158]' : 'text-black/40')">
                                                    <span
                                                        x-text="(simResult.impact.delta_hpp_percentage >= 0 ? '+' : '') + simResult.impact.delta_hpp_percentage.toFixed(1) + '%'"></span>
                                                </td>
                                            </tr>
                            ...
                            <div
                                class="p-4 sm:p-5 rounded-[14px] bg-[#AF52DE]/10 border border-[#AF52DE]/20 flex items-start gap-3.5">
                                <div
                                    class="w-8 h-8 rounded-[8px] bg-[#AF52DE]/15 flex items-center justify-center shrink-0 text-[#AF52DE]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.8"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M12 18v-5.25m0 0a6.01 6.01 0 0 0 1.5-.189m-1.5.189a6.01 6.01 0 0 1-1.5-.189m3.75 7.478a12.06 12.06 0 0 1-4.5 0m3.75 2.383a14.406 14.406 0 0 1-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 1 0-7.516 0c.85.493 1.509 1.333 1.509 2.316V18" />
                                    </svg>
                                </div>
