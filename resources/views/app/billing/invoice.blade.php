...
<body
    class="bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-4 sm:py-8 px-3 sm:px-6 transition-colors">
    ...
    <header
        ...
        <div
            class="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0"
                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
            </svg>
            <span>Klik <strong>"Download PDF Langsung"</strong> untuk mengunduh invoice <code>.pdf</code> resmi tanpa
                dialog printer browser, bebas watermark.</span>
        </div>
    </header>
            ...
            <div class="flex justify-between items-start border-b-2 border-slate-900 pb-5">
                <div class="space-y-2 max-w-md">
                    <div class="flex items-center gap-3 mb-1">
                        ...
                        <div>
                            ...
                            <div class="text-[10px] font-bold text-emerald-700 tracking-wider uppercase mt-0.5">PT Cooca
                ...
                <div class="text-right space-y-0.5">
                    ...
                    <div class="text-[11px] font-bold uppercase tracking-wider text-slate-600 mt-1">
                        STATUS: <span
                            class="font-black {{ $isApproved ? 'text-emerald-700' : ($payment->isRejected() ? 'text-rose-700' : 'text-amber-700') }}">{{ strtoupper(str_replace('_', ' ', $payment->status)) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-8 text-xs pt-1">
                ...
                <div class="space-y-1.5 text-right font-sans">
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <span
                            class="font-semibold text-slate-800">{{ $methodDetails['name'] ?? strtoupper($payment->payment_method) }}</span>
                    </div>
                        ...
                        <div
                            ...
                            @if ($payment->sender_account_name)
                                <div class="pt-2 mt-2 border-t border-slate-200 text-[11px] space-y-0.5">
                                    <div><span class="text-slate-500">Rekening Pengirim:</span> <strong
                                            class="text-slate-900">{{ $payment->sender_account_name }}
