...
@section('content')
    ...
    <div class="space-y-6 pb-12" x-data="{
        ...
        <div
            ...
            <div class="space-y-1.5 max-w-3xl">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span
                        class="rounded-full px-2.5 py-0.5 text-[10px] sm:text-xs font-bold border inline-flex items-center gap-1.5 shadow-2xs {{ $badge['class'] }}">
                ...
                <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                    Selesaikan transfer dana sesuai nominal persis ke rekening resmi, lalu konfirmasi dengan mengunggah
        ...
        <section aria-labelledby="order-progress-heading"
            class="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200/90 dark:border-slate-800/90 shadow-xs p-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 relative">
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-xl bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-500/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center shrink-0 shadow-2xs">
                        <i data-lucide="check" class="w-4 h-4" aria-hidden="true"></i>
                    </div>
                    <div class="min-w-0">
                        <div
                            class="text-[10px] font-mono text-emerald-700 dark:text-emerald-400 uppercase tracking-wider font-bold">
                            Langkah 1</div>
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-xl {{ $payment->isPending() ? 'bg-amber-100 dark:bg-amber-500/20 border border-amber-300 dark:border-amber-500/40 text-amber-700 dark:text-amber-300 ring-2 ring-amber-400/20' : 'bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-500/40 text-emerald-700 dark:text-emerald-400' }} flex items-center justify-center shrink-0 shadow-2xs">
                    ...
                    <div class="min-w-0">
                        <div
                            class="text-[10px] font-mono {{ $payment->isPending() ? 'text-amber-700 dark:text-amber-400' : 'text-emerald-700 dark:text-emerald-400' }} uppercase tracking-wider font-bold">
                            Langkah 2</div>
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-xl {{ $payment->isAwaitingApproval() ? 'bg-cyan-100 dark:bg-cyan-500/20 border border-cyan-300 dark:border-cyan-500/40 text-cyan-700 dark:text-cyan-300 ring-2 ring-cyan-400/20' : ($payment->isApproved() ? 'bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-500/40 text-emerald-700 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-slate-400') }} flex items-center justify-center shrink-0 shadow-2xs">
                    ...
                    <div class="min-w-0">
                        <div
                            class="text-[10px] font-mono {{ $payment->isAwaitingApproval() ? 'text-cyan-700 dark:text-cyan-400' : ($payment->isApproved() ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-400') }} uppercase tracking-wider font-bold">
                            Langkah 3</div>
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-xl {{ $payment->isApproved() ? 'bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-500/40 text-emerald-700 dark:text-emerald-400 ring-2 ring-emerald-500/20' : 'bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-slate-400' }} flex items-center justify-center shrink-0 shadow-2xs">
                        <i data-lucide="sparkles" class="w-4 h-4" aria-hidden="true"></i>
                    </div>
                    <div class="min-w-0">
                        <div
                            class="text-[10px] font-mono {{ $payment->isApproved() ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-400' }} uppercase tracking-wider font-bold">
                            Langkah 4</div>
        ...
        @if ($payment->isRejected())
            <section aria-labelledby="status-rejected-heading"
                class="p-5 sm:p-6 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border-2 border-rose-200 dark:border-rose-500/50 shadow-xs flex flex-col sm:flex-row items-start gap-5">
                <div class="w-11 h-11 rounded-xl bg-rose-100 dark:bg-rose-500/20 border border-rose-200 dark:border-rose-500/40 text-rose-600 dark:text-rose-400 flex items-center justify-center shrink-0 shadow-2xs"
                    aria-hidden="true">
                ...
                <div class="space-y-2 flex-1">
                    <div class="flex items-center gap-2">
                        <span
                            class="text-xs font-mono font-bold uppercase tracking-wider text-rose-700 dark:text-rose-400">Pemberitahuan
                            Status Tagihan</span>
                        <span
                            class="text-[11px] text-rose-600 dark:text-rose-300 font-mono">{{ $payment->rejected_at?->format('d M Y H:i') }}</span>
                    </div>
                    <h4 id="status-rejected-heading" class="font-black text-slate-900 dark:text-white text-base">
                        Pembayaran Belum Dapat Diverifikasi</h4>
                    <div
                        class="p-3.5 rounded-xl bg-white dark:bg-slate-950/80 border border-rose-200 dark:border-rose-500/30 text-xs text-rose-800 dark:text-rose-200">
                        <strong class="text-slate-900 dark:text-white">Alasan Penolakan:</strong>
                        {{ $payment->admin_notes ?? 'Nominal transfer tidak sesuai dengan kode unik atau mutasi bank belum teridentifikasi.' }}
                    </div>
                    <p class="text-xs text-slate-600 dark:text-slate-300">
                        Jangan khawatir, Anda dapat melakukan transfer ulang atau mengunggah foto struk transfer yang benar
        ...
        @if ($payment->isAwaitingApproval())
            <section aria-labelledby="status-awaiting-heading"
                class="p-5 sm:p-6 rounded-2xl bg-cyan-50 dark:bg-gradient-to-r dark:from-cyan-950/50 dark:via-slate-900/60 dark:to-cyan-950/50 border-2 border-cyan-200 dark:border-cyan-500/40 shadow-xs flex flex-col sm:flex-row items-start gap-5">
                <div class="w-11 h-11 rounded-xl bg-cyan-100 dark:bg-cyan-500/20 border border-cyan-200 dark:border-cyan-500/40 text-cyan-600 dark:text-cyan-300 flex items-center justify-center shrink-0 shadow-2xs"
                    aria-hidden="true">
                ...
                <div class="space-y-2 flex-1">
                    <div class="flex items-center gap-2">
                        <span
                            class="text-xs font-mono font-bold uppercase tracking-wider text-cyan-700 dark:text-cyan-400">Sedang
                            Diproses</span>
                        <span class="text-[11px] text-cyan-600 dark:text-cyan-300 font-mono">Estimasi SLA: 5–15
                            Menit</span>
                    </div>
                    <h4 id="status-awaiting-heading" class="font-black text-slate-900 dark:text-white text-base">Bukti
                        Pembayaran Anda Berhasil Diterima</h4>
        ...
        @if ($payment->isApproved())
            <section aria-labelledby="status-approved-heading"
                class="p-5 sm:p-6 rounded-2xl bg-emerald-50 dark:bg-gradient-to-r dark:from-emerald-950/60 dark:via-slate-900/60 dark:to-emerald-950/60 border-2 border-emerald-200 dark:border-emerald-500/50 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-6">
                <div class="flex items-start gap-5">
                    <div class="w-12 h-12 rounded-xl bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-500/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center shrink-0 shadow-2xs"
                        aria-hidden="true">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2">
                            <span
                                class="text-xs font-mono font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">Pembayaran
                                Sah</span>
                            <span class="text-[11px] text-emerald-600 dark:text-emerald-300 font-mono">Disetujui
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            ...
            <div class="lg:col-span-7 space-y-6">
                ...
                <section aria-labelledby="nominal-transfer-heading"
                    class="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200/90 dark:border-slate-800/90 shadow-xs p-5 sm:p-6 relative overflow-hidden">
                    <div class="absolute -top-12 -right-12 w-48 h-48 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"
                        aria-hidden="true"></div>

                    <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-4">
                        <div>
                            <span
                                class="text-[10px] font-mono uppercase tracking-widest text-slate-500 dark:text-slate-400 font-bold block mb-1">Total
                                Pembayaran Transfer</span>
                            <h3 id="nominal-transfer-heading" class="text-xs text-slate-600 dark:text-slate-300">Wajib
                                transfer presisi tepat hingga 3 digit terakhir:</h3>
                        </div>
                        <span
                            class="rounded-full px-2.5 py-0.5 text-[10px] font-bold border inline-flex items-center gap-1 bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 border-amber-200/90 dark:border-amber-800/90 font-mono">
                    ...
                    <div class="py-5 flex flex-col sm:flex-row sm:items-baseline justify-between gap-4">
                        <div>
                            <div
                                class="text-3xl sm:text-4xl font-black font-mono tracking-tight text-slate-900 dark:text-white flex items-baseline gap-1">
                            ...
                            <div class="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1.5">
                                <i data-lucide="clock" class="w-3.5 h-3.5 text-amber-500 dark:text-amber-400"
                                    aria-hidden="true"></i>
                        ...
                        <div class="flex items-center gap-2">
                            <button type="button"
                                ...
                                <i data-lucide="copy" class="w-4 h-4" x-show="copiedText !== 'nominal'"
                                    aria-hidden="true"></i>
                                <i data-lucide="check" class="w-4 h-4 text-emerald-600 dark:text-emerald-400"
                    ...
                    <div
                        class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 space-y-2.5 font-mono">
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-slate-600 dark:text-slate-400 font-sans">Harga Paket Dasar:</span>
                            <span class="font-bold text-slate-900 dark:text-slate-200">Rp
                                {{ number_format($payment->amount, 0, ',', '.') }}</span>
                        </div>
                        <div
                            class="flex items-center justify-between text-xs pt-1 border-t border-slate-200/80 dark:border-slate-900">
                            ...
                            <span
                                class="font-black text-amber-700 dark:text-amber-400 bg-amber-100/80 dark:bg-amber-500/10 px-2 py-0.5 rounded border border-amber-200 dark:border-amber-500/30">
                                +{{ $uniqueStr }}
                            </span>
                        </div>
                        <div
                            class="flex items-center justify-between text-xs pt-1 border-t border-slate-200 dark:border-slate-800 font-bold">
                            <span class="text-slate-900 dark:text-white font-sans">Total Tagihan Final:</span>
                            <span class="text-emerald-600 dark:text-emerald-400 font-black text-sm">Rp
                    ...
                    <!-- Warning Notice -->
                    <div
                        class="mt-4 p-3.5 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 flex items-start gap-3">
                        <i data-lucide="info" class="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5"
                            aria-hidden="true"></i>
                        <p class="text-[11px] text-amber-800 dark:text-amber-200/90 leading-relaxed">
                            <strong class="font-semibold">Penting:</strong> Mohon transfer dengan jumlah yang <strong
                                class="underline decoration-amber-400 font-bold">PERSIS</strong> hingga 3 digit terakhir
                            (<span
                                class="font-mono font-bold text-amber-700 dark:text-amber-300">{{ $uniqueStr }}</span>).
                            Perbedaan nominal akan memperlambat pencocokan mutasi rekening Anda.
                        </p>
                    </div>
                </section>
                ...
                <section aria-labelledby="destination-account-heading"
                    ...
                    <div class="border-b border-slate-100 dark:border-slate-800/80 pb-4 flex items-center justify-between">
                        <div>
                            <span
                                class="text-[10px] font-mono uppercase tracking-widest text-slate-500 dark:text-slate-400 font-bold block mb-1">Rekening
                                Tujuan Resmi</span>
                            <h3 id="destination-account-heading"
                                ...
                                <i data-lucide="building-2" class="w-4 h-4 text-emerald-600 dark:text-emerald-400"
                                    aria-hidden="true"></i>
                                <span>{{ $methodDetails['bank_name'] }}</span>
                            </h3>
                        </div>
                        <span
                            class="rounded-full px-2.5 py-0.5 text-[10px] font-bold border inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 font-mono uppercase tracking-wider">
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300">Nomor Rekening / Saluran
                            Tujuan:</label>
                        <div
                            class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-inner">
                            <div class="space-y-0.5">
                                <div
                                    class="text-xl sm:text-2xl font-black font-mono text-slate-900 dark:text-white tracking-wider selection:bg-emerald-500 selection:text-white">
                                    {{ $methodDetails['account_number'] }}
                                </div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">
                                    a/n <span
                                        class="font-bold text-slate-800 dark:text-slate-200">{{ $methodDetails['account_name'] }}</span>
                                </div>
                            </div>
                            ...
                            @if (($methodDetails['type'] ?? '') !== 'qris')
                                <button type="button"
                                    ...
                                    <i data-lucide="copy" class="w-3.5 h-3.5" x-show="copiedText !== 'rekening'"
                                        aria-hidden="true"></i>
                                    <i data-lucide="check" class="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400"
                                        x-show="copiedText === 'rekening'" x-cloak aria-hidden="true"></i>
                                    <span
                                        x-text="copiedText === 'rekening' ? 'Nomor Tersalin!' : 'Salin Nomor Rekening'"></span>
                                </button>
                            @endif
                        </div>
                    </div>
                    ...
                    @if (($methodDetails['type'] ?? '') === 'qris' || $payment->payment_method === 'qris')
                        <div
                            class="p-6 rounded-2xl bg-white border border-slate-200 text-slate-950 text-center space-y-4 shadow-sm">
                            <div class="flex items-center justify-between border-b border-slate-200 pb-3">
                                <div class="text-left">
                                    <span
                                        class="text-[9px] font-black tracking-widest text-slate-500 uppercase block font-mono">STANDAR
                                        PEMBAYARAN NASIONAL</span>
                                    <span class="text-sm font-black text-slate-900 font-sans tracking-tight">QRIS COOCA PAY
                                        INDONESIA</span>
                                </div>
                                <span
                                    class="px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 text-[10px] font-bold font-mono">NMID:
                                    ID1020304050</span>
                            </div>

                            <div
                                class="p-4 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-300 inline-block shadow-inner">
                                ...
                                @else
                                    <div class="w-56 h-56 flex flex-col items-center justify-center text-center p-3">
                                        <i data-lucide="qr-code" class="w-36 h-36 text-slate-900 mx-auto"
                                            aria-hidden="true"></i>
                                        <span
                                            class="text-[10px] font-mono font-bold text-slate-700 mt-2 block">{{ $methodDetails['account_number'] }}</span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="space-y-1 text-xs text-slate-600 max-w-sm mx-auto">
                                ...
                                <p class="text-[11px] text-slate-500">BCA, Mandiri Livin, BRImo, BNI+, GoPay, OVO, Dana,
                                    ShopeePay, LinkAja.</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if (!empty($methodDetails['instructions']))
                        <div
                            class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-xs space-y-1.5">
                            <div class="flex items-center gap-2 font-bold text-slate-800 dark:text-slate-300">
                                <i data-lucide="help-circle" class="w-4 h-4 text-cyan-600 dark:text-cyan-400"
                                    aria-hidden="true"></i>
                    ...
                    <div
                        class="pt-2 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                ...
                <section aria-labelledby="confirmation-heading"
                    ...
                    <div class="border-b border-slate-100 dark:border-slate-800 pb-4">
                        <h3 id="confirmation-heading"
                            class="text-sm sm:text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
                            <i data-lucide="receipt-text" class="w-4 h-4 text-emerald-600 dark:text-emerald-400"
                                aria-hidden="true"></i>
                    ...
                    @if ($payment->isPending() || $payment->isRejected())
                        @if (\App\Support\Context::hasPermission('billing.manage'))
                            ...
                            <form method="POST" action="{{ route('billing.payment.upload', $payment) }}"
                                ...
                                <div x-show="fileError" x-cloak
                                    class="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-300 text-xs flex items-start gap-2">
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative group" @dragover.prevent="isDragging = true"
                                        ...
                                        <div class="p-6 rounded-2xl border-2 border-dashed transition-all text-center space-y-2"
                                            :class="isDragging ? 'border-emerald-500 bg-emerald-50/50 dark:bg-emerald-950/20' :
                                                'border-slate-200 dark:border-slate-700 group-hover:border-emerald-400 bg-slate-50/50 dark:bg-slate-950/60 group-hover:bg-slate-50 dark:group-hover:bg-slate-950'">
                                            <div class="w-12 h-12 rounded-xl bg-white dark:bg-slate-800/80 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-500/20 text-slate-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 flex items-center justify-center mx-auto transition shadow-2xs"
                                                aria-hidden="true">
                                                <i data-lucide="upload-cloud" class="w-6 h-6"></i>
                                            </div>
                                            <div class="text-xs text-slate-700 dark:text-slate-300 font-semibold">
                                                <span
                                                    class="text-emerald-600 dark:text-emerald-400 underline decoration-emerald-500/50 font-bold">Pilih
                                                    berkas</span> atau geser ke sini
                                            </div>
                                            <p class="text-[10px] text-slate-500 dark:text-slate-400 font-mono">JPG, PNG,
                                    ...
                                    <div x-show="proofPreview" x-cloak
                                        class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-emerald-300 dark:border-emerald-500/40 space-y-3 mt-2">
                                        <div class="flex items-center justify-between text-xs">
                                            <div class="flex items-center gap-2 truncate">
                                                <i data-lucide="file-text"
                                                    ...
                                                    aria-hidden="true"></i>
                                                <span
                                                    class="font-mono text-slate-800 dark:text-slate-200 truncate font-semibold"
                                            ...
                                            <div class="flex items-center gap-2 shrink-0">
                                                ...
                                                <button type="button" @click="clearFile()"
                                                    class="p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 hover:text-rose-500 transition cursor-pointer"
                                                    title="Hapus Berkas">
                                        ...
                                        <template x-if="proofPreview && proofPreview !== 'pdf'">
                                            <div
                                                class="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-black/5 dark:bg-black/40">
                                        ...
                                        <template x-if="proofPreview === 'pdf'">
                                            <div
                                                class="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-center flex items-center justify-center gap-2 text-xs text-slate-700 dark:text-slate-300 font-mono">
                                                <i data-lucide="file" class="w-4 h-4 text-rose-500"
                                                    aria-hidden="true"></i>
                                ...
                                <div class="space-y-1.5">
                                    <label for="sender_bank_input"
                                        class="block text-xs font-bold text-slate-700 dark:text-slate-300">Bank / e-Wallet
                                        Pengirim</label>
                                    ...
                                    <div class="flex flex-wrap gap-1.5 pt-1" aria-label="Pilihan Cepat Bank">
                                        <button type="button" @click="selectedQuickBank = 'BCA'"
                                            class="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold transition cursor-pointer">BCA</button>
                                        <button type="button" @click="selectedQuickBank = 'Mandiri'"
                                            class="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold transition cursor-pointer">Mandiri</button>
                                        <button type="button" @click="selectedQuickBank = 'BRI'"
                                            class="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold transition cursor-pointer">BRI</button>
                                        <button type="button" @click="selectedQuickBank = 'BNI'"
                                            class="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold transition cursor-pointer">BNI</button>
                                        <button type="button" @click="selectedQuickBank = 'GoPay'"
                                            class="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold transition cursor-pointer">GoPay</button>
                                        <button type="button" @click="selectedQuickBank = 'OVO'"
                                            class="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold transition cursor-pointer">OVO</button>
                                        <button type="button" @click="selectedQuickBank = 'Dana'"
                                            class="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold transition cursor-pointer">Dana</button>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-1.5">
                                    <label for="sender_account_number_input"
                                        ...
                                        Nomor Rekening / No HP Pengirim <span
                                            class="text-slate-500 text-[11px] font-normal">(Opsional)</span>
                                    </label>
                                ...
                                <button type="submit" :disabled="isUploading"
                                    ...
                                    <i data-lucide="send" class="w-4 h-4 group-hover:translate-x-0.5 transition-transform"
                        ...
                        @else
                            <div
                                class="p-6 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 text-xs text-center space-y-2">
                                <i data-lucide="lock" class="w-8 h-8 text-amber-500 mx-auto" aria-hidden="true"></i>
                                <div class="font-bold text-slate-800 dark:text-white">Akses Terbatas</div>
                    ...
                    @else
                        ...
                        <div class="space-y-4 text-xs">
                            <div
                                class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 space-y-2.5">
                                <div
                                    class="flex items-center justify-between pb-2 border-b border-slate-200/80 dark:border-slate-900">
                                    <span class="text-slate-500 dark:text-slate-400">Pengirim:</span>
                                    <span
                                        class="font-bold text-slate-900 dark:text-white font-mono">{{ $payment->sender_account_name }}</span>
                                </div>
                                <div
                                    class="flex items-center justify-between pb-2 border-b border-slate-200/80 dark:border-slate-900">
                                    <span class="text-slate-500 dark:text-slate-400">Saluran Bank:</span>
                                    <span
                                        class="font-bold text-slate-800 dark:text-slate-200">{{ $payment->sender_bank ?: '-' }}</span>
                                </div>
                                <div
                                    class="flex items-center justify-between pb-2 border-b border-slate-200/80 dark:border-slate-900">
                                    <span class="text-slate-500 dark:text-slate-400">Waktu Kirim:</span>
                                    <span
                                        class="font-mono text-slate-700 dark:text-slate-300">{{ $payment->proof_uploaded_at?->format('d M Y, H:i') }}
                                        WIB</span>
                                </div>
                                @if ($payment->notes)
                                    <div class="pt-1">
                                        <span class="text-slate-500 dark:text-slate-400 block mb-1">Catatan
                                            Tambahan:</span>
                                        <p
                                            class="text-slate-700 dark:text-slate-300 italic text-[11px] bg-white dark:bg-slate-900/60 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800">
                                            {{ $payment->notes }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            @if ($payment->payment_proof_path)
                                <div
                                    class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-center space-y-3">
                                    <div class="flex items-center justify-between">
                                        <span
                                            class="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider font-mono">Lampiran
                                            Struk Transfer</span>
                                        <a href="{{ $payment->getProofUrl() }}" target="_blank"
                                            class="text-xs text-emerald-600 dark:text-emerald-400 hover:underline inline-flex items-center gap-1 font-bold focus-visible:ring-1 focus-visible:ring-emerald-400 rounded">
                                            <span>Buka Full</span>
                                            <i data-lucide="external-link" class="w-3.5 h-3.5" aria-hidden="true"></i>
                                        </a>
                                    </div>
                                    <div
                                        class="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-white dark:bg-black/50 p-2">
                                        <a href="{{ $payment->getProofUrl() }}" target="_blank"
                                            class="block group overflow-hidden rounded-lg">
                                            <img src="{{ $payment->getProofUrl() }}" alt="Bukti Transfer Struk"
                                                class="max-h-64 mx-auto object-contain group-hover:scale-105 transition-transform duration-300">
                                        </a>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div
                                class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 text-[11px] leading-relaxed space-y-1">
