...
@section('content')
    <div class="space-y-6 pb-28 lg:pb-10" x-data="{
        ...
        <div
            ...
            <div class="space-y-2 max-w-3xl">
                <div class="flex items-center gap-2">
                    <span
                        class="rounded-[10px] px-2.5 py-1 text-xs font-bold border inline-flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 font-mono tabular-nums">
                ...
                <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                    Pantau seluruh rekam jejak pesanan langganan SaaS, bukti mutasi transfer bank, invoice resmi, dan status
                    verifikasi akun.
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
                <a href="{{ route('billing.limits') }}"
                    class="px-4 py-2.5 rounded-[12px] text-xs font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/80 active:scale-[0.98] transition cursor-pointer shadow-2xs inline-flex items-center justify-center gap-2 flex-1 sm:flex-none">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
            ...
            <div
                ...
                <div>
                    ...
                    <div
                        class="text-xl sm:text-2xl font-black text-slate-900 dark:text-white font-mono tracking-tight tabular-nums truncate">
                ...
                <div
                    ...
                    <span class="font-bold text-slate-700 dark:text-slate-300 font-mono tabular-nums">{{ $payments->count() }}
                        Ditampilkan</span>
                </div>
            </div>
            ...
            <div
                ...
                <div
                    ...
                    <span
                        class="font-bold text-emerald-600 dark:text-emerald-400 font-mono tabular-nums">{{ $business->subscription?->ends_at?->format('d/m/Y') ?? 'Selamanya' }}</span>
                </div>
            </div>
            ...
            <div
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        <span
                            class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Workspace
                            Bisnis</span>
                        <div
                ...
                <div
                    ...
                    <span class="font-bold text-slate-700 dark:text-slate-300 font-mono tabular-nums">ID #{{ $business->id }}</span>
                </div>
            </div>
            ...
            <div
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        <span
                            class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Verifikasi
                            Mutasi</span>
                        <div
        ...
        <div
            class="bg-white dark:bg-slate-900 rounded-[20px] border border-black/[0.06] dark:border-white/[0.08] shadow-xs p-3.5 sm:p-4 space-y-3">
            <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
                <!-- Search Input: Anti auto-zoom text-[16px] sm:text-[14px] -->
                <div class="relative flex-1 max-w-md">
                    <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2"
                        aria-hidden="true"></i>
                    <input type="text" x-model="searchQuery" placeholder="Cari nomor order atau nama paket..."
                        class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-[12px] pl-9 pr-3.5 py-2 text-[16px] sm:text-[14px] text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-hidden focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition">
                </div>

                <!-- Status Filter: Segmented Control -->
                <div class="inline-flex p-1 bg-black/[0.04] dark:bg-white/[0.06] rounded-[14px] border border-black/[0.04] dark:border-white/[0.06] overflow-x-auto max-w-full gap-1"
                    role="tablist" aria-label="Filter Status Tagihan">
                    <button type="button" @click="statusFilter = 'all'"
                        class="px-3 py-1.5 rounded-[10px] text-xs font-bold transition cursor-pointer shrink-0 active:scale-[0.98] focus-visible:ring-1 focus-visible:ring-emerald-400"
                        :class="statusFilter === 'all' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-xs' :
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'">
                        Semua
                    </button>
                    <button type="button" @click="statusFilter = 'pending'"
                        class="px-3 py-1.5 rounded-[10px] text-xs font-bold transition cursor-pointer shrink-0 active:scale-[0.98] focus-visible:ring-1 focus-visible:ring-amber-400"
                        :class="statusFilter === 'pending' ? 'bg-white dark:bg-slate-800 text-amber-600 dark:text-amber-400 shadow-xs' :
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'">
                        Menunggu Bayar
                    </button>
                    <button type="button" @click="statusFilter = 'awaiting_approval'"
                        class="px-3 py-1.5 rounded-[10px] text-xs font-bold transition cursor-pointer shrink-0 active:scale-[0.98] focus-visible:ring-1 focus-visible:ring-cyan-400"
                        :class="statusFilter === 'awaiting_approval' ? 'bg-white dark:bg-slate-800 text-cyan-600 dark:text-cyan-400 shadow-xs' :
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'">
                        Verifikasi
                    </button>
                    <button type="button" @click="statusFilter = 'approved'"
                        class="px-3 py-1.5 rounded-[10px] text-xs font-bold transition cursor-pointer shrink-0 active:scale-[0.98] focus-visible:ring-1 focus-visible:ring-emerald-400"
                        :class="statusFilter === 'approved' ? 'bg-white dark:bg-slate-800 text-emerald-600 dark:text-emerald-400 shadow-xs' :
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'">
                        Berhasil
                    </button>
                    <button type="button" @click="statusFilter = 'rejected'"
                        class="px-3 py-1.5 rounded-[10px] text-xs font-bold transition cursor-pointer shrink-0 active:scale-[0.98] focus-visible:ring-1 focus-visible:ring-rose-400"
                        :class="statusFilter === 'rejected' ? 'bg-white dark:bg-slate-800 text-rose-600 dark:text-rose-400 shadow-xs' :
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'">
                        Ditolak
                    </button>
                </div>
        ...
        <div
            class="bg-white dark:bg-slate-900 rounded-[20px] border border-black/[0.06] dark:border-white/[0.08] shadow-xs overflow-hidden">
                                ...
                                <tr x-show="matchesFilter('{{ $p->order_number }}', '{{ $pPackageName }}', '{{ $p->status }}')"
                                    ...
                                    <td class="py-3.5 px-5">
                                        <div
                                            ...
                                            <i data-lucide="hash" class="w-3.5 h-3.5 text-slate-400"
                                                aria-hidden="true"></i>
                                    ...
                                    <td class="py-3.5 px-4">
                                        ...
                                        <div
                                            class="text-[10px] text-slate-500 dark:text-slate-400 font-mono uppercase mt-0.5">
                                    ...
                                    <td class="py-3.5 px-4 text-right">
                                        <div class="font-mono font-bold text-sm text-emerald-600 dark:text-emerald-400 tabular-nums">
                                            Rp {{ number_format($p->total_payable, 0, ',', '.') }}
                                        </div>
                                        @if ($p->unique_code > 0)
                                            <div
                                                class="text-[10px] text-amber-600 dark:text-amber-400 font-mono font-semibold tabular-nums">
                                    ...
                                    <td class="py-3.5 px-4 text-center">
                                        @if ($p->payment_proof_path)
                                            <a href="{{ $p->getProofUrl() }}" target="_blank"
                                                class="inline-flex items-center gap-1.5 text-xs text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 font-bold transition focus-visible:ring-1 focus-visible:ring-cyan-400 rounded">
                                                <i data-lucide="image" class="w-3.5 h-3.5" aria-hidden="true"></i>
                                                <span>Lihat Bukti</span>
                                            </a>
                                        @else
                                            <span class="text-slate-400 dark:text-slate-500 text-[11px] italic">Belum
                                                diunggah</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="py-3.5 px-4 text-center">
                                        <span
                                            class="rounded-[8px] px-2.5 py-1 text-[10px] sm:text-xs font-bold border inline-flex items-center gap-1 shadow-2xs {{ $badge['class'] }}">
                                    ...
                                    <!-- Date -->
                                    <td class="py-3.5 px-4 font-mono text-slate-600 dark:text-slate-400 text-[11px] tabular-nums">
                                    ...
                                    <td class="py-3.5 px-5 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('billing.payment.invoice', $p) }}" target="_blank"
                                                ...
                                                class="p-2 rounded-[10px] text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 active:scale-[0.98] transition cursor-pointer">
                                                <i data-lucide="printer"
                                                    class="w-4 h-4 text-emerald-600 dark:text-emerald-400"
                                                    aria-hidden="true"></i>
                                            </a>
                                            <a href="{{ route('billing.payment.show', $p) }}"
                                                class="px-3 py-1.5 rounded-[10px] text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 active:scale-[0.98] transition cursor-pointer inline-flex items-center gap-1">
                                                <span>Detail</span>
                        ...
                        <div x-show="matchesFilter('{{ $p->order_number }}', '{{ $pPackageName }}', '{{ $p->status }}')"
                            ...
                            <div class="flex items-center justify-between gap-2">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span
                                        class="font-mono font-bold text-sm text-slate-900 dark:text-white">{{ $p->order_number }}</span>
                                </div>
                                <span
                                    class="rounded-[8px] px-2.5 py-0.5 text-[10px] font-bold border inline-flex items-center gap-1 {{ $badge['class'] }}">
                            ...
                            <div
                                class="p-3.5 rounded-[14px] bg-slate-50 dark:bg-slate-950/70 border border-slate-200/80 dark:border-slate-800/80 space-y-2">
                                ...
                                <div class="flex items-center justify-between text-xs">
                                    ...
                                    <span
                                        class="font-semibold text-slate-800 dark:text-slate-200">{{ $method['name'] ?? strtoupper($p->payment_method) }}</span>
                                </div>
                                <div
                                    class="flex items-center justify-between text-xs pt-2 border-t border-slate-200/80 dark:border-slate-900">
                                    ...
                                    <div class="text-right">
                                        ...
                                        @if ($p->unique_code > 0)
                                            <div class="text-[10px] text-amber-600 dark:text-amber-400 font-mono tabular-nums">
                                                Kode unik: +{{ str_pad((string) $p->unique_code, 3, '0', STR_PAD_LEFT) }}
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <div class="flex items-center justify-between gap-3 pt-1">
                                <div
                                    class="text-[11px] font-mono text-slate-500 dark:text-slate-400 flex items-center gap-1 tabular-nums">
                                ...
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('billing.payment.invoice', $p) }}" target="_blank"
                                        class="p-2 rounded-[10px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-emerald-600 dark:text-emerald-400 text-xs font-bold transition shadow-2xs active:scale-[0.98] focus-visible:ring-1 focus-visible:ring-emerald-400"
                                    ...
                                    @if ($p->payment_proof_path)
                                        <a href="{{ $p->getProofUrl() }}" target="_blank"
                                            class="p-2 rounded-[10px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-cyan-600 dark:text-cyan-400 text-xs font-bold active:scale-[0.98] focus-visible:ring-1 focus-visible:ring-cyan-400"
                                    ...
                                    <a href="{{ route('billing.payment.show', $p) }}"
                                        class="px-3.5 py-2 rounded-[10px] text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 shadow-sm shadow-emerald-600/20 active:scale-[0.98] transition cursor-pointer flex items-center gap-1">
                                        <span>Rincian</span>
            ...
            @else
                ...
                <div class="py-16 px-6 text-center space-y-4">
                    <div class="w-16 h-16 rounded-[16px] bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 flex items-center justify-center mx-auto shadow-2xs"
                        aria-hidden="true">
                    ...
                    @if (\App\Support\Context::hasPermission('billing.manage'))
                        <div class="pt-2">
                            <a href="{{ route('billing.checkout') }}"
                                class="px-4 py-2.5 rounded-[12px] text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 shadow-sm shadow-emerald-600/20 active:scale-[0.98] transition cursor-pointer inline-flex items-center gap-2 focus-visible:ring-2 focus-visible:ring-emerald-500">
