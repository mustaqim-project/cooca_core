...
@section('content')
    <div class="space-y-6 pb-12" x-data="{
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div
                ...
                <div
                    ...
                    <span class="font-bold text-slate-700 dark:text-slate-300 font-mono">{{ $payments->count() }}
                        Ditampilkan</span>
                </div>
            </div>
            ...
            <div
                ...
                <div
                    ...
                    <span
                        class="font-bold text-emerald-600 dark:text-emerald-400 font-mono">{{ $business->subscription?->ends_at?->format('d/m/Y') ?? 'Selamanya' }}</span>
                </div>
            </div>
        ...
        <div
            class="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200/90 dark:border-slate-800/90 shadow-xs p-3.5 sm:p-4 space-y-3">
            <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
                ...
                <div class="flex flex-wrap items-center gap-1.5" role="tablist" aria-label="Filter Status Tagihan">
                    <button type="button" @click="statusFilter = 'all'"
                        ...
                        :class="statusFilter === 'all' ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-600/20' :
                    ...
                    <button type="button" @click="statusFilter = 'pending'"
                        ...
                        :class="statusFilter === 'pending' ? 'bg-amber-500 text-slate-950 shadow-sm shadow-amber-500/20' :
                            'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700'">
                        Menunggu Bayar
                    </button>
                    <button type="button" @click="statusFilter = 'awaiting_approval'"
                        ...
                        :class="statusFilter === 'awaiting_approval' ? 'bg-cyan-600 text-white shadow-sm shadow-cyan-600/20' :
                            'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700'">
                        Verifikasi
                    </button>
                    <button type="button" @click="statusFilter = 'approved'"
                        class="px-3 py-1.5 rounded-xl text-xs font-bold transition cursor-pointer focus-visible:ring-1 focus-visible:ring-emerald-400"
                        :class="statusFilter === 'approved' ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-600/20' :
                            'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700'">
                        Berhasil
                    </button>
                    <button type="button" @click="statusFilter = 'rejected'"
                        ...
                        :class="statusFilter === 'rejected' ? 'bg-rose-600 text-white shadow-sm shadow-rose-600/20' :
                            'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700'">
                        Ditolak
                    </button>
                </div>
        ...
        <div
                            ...
                            @foreach ($payments as $p)
                                ...
                                <tr x-show="matchesFilter('{{ $p->order_number }}', '{{ $pPackageName }}', '{{ $p->status }}')"
                                    ...
                                    <td class="py-3.5 px-4 text-right">
                                        ...
                                        @if ($p->unique_code > 0)
                                            <div
                                                class="text-[10px] text-amber-600 dark:text-amber-400 font-mono font-semibold">
                                    ...
                                    <td class="py-3.5 px-4 text-center">
                                        <span
                                            class="rounded-full px-2.5 py-0.5 text-[10px] sm:text-xs font-bold border inline-flex items-center gap-1 shadow-2xs {{ $badge['class'] }}">
                    ...
                    @foreach ($payments as $p)
                        ...
                        <div x-show="matchesFilter('{{ $p->order_number }}', '{{ $pPackageName }}', '{{ $p->status }}')"
                            ...
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span
                                    class="rounded-full px-2.5 py-0.5 text-[10px] font-bold border inline-flex items-center gap-1 {{ $badge['class'] }}">
                            ...
                            <div
                                class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200/80 dark:border-slate-800/80 space-y-2">
                            ...
                            <div class="flex items-center justify-between gap-3 pt-1">
                                <div
                                    class="text-[11px] font-mono text-slate-500 dark:text-slate-400 flex items-center gap-1">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($p->payment_proof_path)
                                        <a href="{{ $p->getProofUrl() }}" target="_blank"
                                            class="p-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-cyan-600 dark:text-cyan-400 text-xs font-bold focus-visible:ring-1 focus-visible:ring-cyan-400"
            ...
            @else
                ...
                <div class="py-16 px-6 text-center space-y-4">
                    <div class="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 flex items-center justify-center mx-auto shadow-2xs"
                        aria-hidden="true">
