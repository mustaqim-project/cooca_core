        ...
        <header
            ...
            <div>
                <nav class="flex items-center gap-1.5 text-[12px] text-black/50 dark:text-white/50 mb-1">
                    ...
                    <span>›</span>
                    <a href="{{ route('finance.cash-bank.index') }}" class="hover:text-[#007AFF] transition-colors">Kas &
                        Bank</a>
                    <span>›</span>
                    <span class="text-black dark:text-white font-medium">Buku Kas & Ledger</span>
                </nav>
                <h1 class="text-[20px] font-semibold text-black dark:text-white tracking-tight">Buku Kas & Ledger Rekening
                </h1>
                <p class="text-[13px] text-black/50 dark:text-white/50">Riwayat kronologis mutasi debit dan kredit seluruh
                    akun kas dan rekening bank</p>
            </div>
            <div class="flex items-center gap-2 w-full sm:w-auto">
                @if (\App\Support\Context::hasPermission('finance.cash_bank'))
                    <a href="{{ route('finance.cash-bank.index') }}"
                        class="h-9 px-4 rounded-[10px] text-[13px] font-medium text-black/80 dark:text-white/80 bg-black/[0.06] dark:bg-white/[0.08] hover:bg-black/[0.09] dark:hover:bg-white/[0.12] active:scale-[0.97] transition-all flex items-center gap-1.5">
            ...
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <form method="GET" action="{{ route('finance.cash-bank.ledger') }}"
                    ...
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-black/50 dark:text-white/50" fill="none" stroke="currentColor"
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M12 3c2.755 0 5.455.232 8.083.678.539.092.917.56.917 1.109v1.244a2.25 2.25 0 01-.659 1.591l-5.432 5.432a2.25 2.25 0 00-.659 1.591v2.927a2.25 2.25 0 01-1.244 2.013L9.75 21v-6.568a2.25 2.25 0 00-.659-1.591L3.659 7.422A2.25 2.25 0 013 5.831V4.787c0-.548.378-1.017.917-1.109A48.834 48.834 0 0112 3z" />
                        </svg>
                        <span class="text-[13px] font-semibold text-black/80 dark:text-white/80">Filter:</span>
                    </div>
                    <select name="account_id"
                        class="h-9 px-3 bg-[#F2F2F7] dark:bg-[#2C2C2E] border border-black/5 dark:border-white/10 rounded-[10px] text-[13px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF] transition-all">
                    ...
                    <input type="date" name="start_date" value="{{ request('start_date') }}"
                        class="h-9 px-2.5 bg-[#F2F2F7] dark:bg-[#2C2C2E] border border-black/5 dark:border-white/10 rounded-[10px] text-[12px] text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari keterangan..."
                        class="h-9 px-3 bg-[#F2F2F7] dark:bg-[#2C2C2E] border border-black/5 dark:border-white/10 rounded-[10px] text-[13px] text-black dark:text-white placeholder:text-black/35 dark:placeholder:text-white/35 focus:outline-none focus:ring-2 focus:ring-[#007AFF]">
                    <button type="submit"
                        class="h-9 px-3.5 rounded-[10px] bg-[#007AFF] text-white text-[13px] font-medium hover:bg-[#0071E3] transition-colors">Terapkan</button>
                ...
                @if ($account)
                    <div
                        class="flex items-center gap-3 bg-[#F2F2F7]/80 dark:bg-[#2C2C2E]/80 border border-black/5 dark:border-white/5 rounded-[12px] px-4 py-2 shrink-0">
                        <div>
                            <span
                                class="text-[11px] uppercase font-semibold text-black/50 dark:text-white/50 tracking-wider">Saldo
                                Terpilih</span>
                            <div
                                class="text-[18px] font-bold tabular-nums text-[#34C759] dark:text-[#30D158] tracking-tight">
                                Rp {{ number_format($account->current_balance, 0, ',', '.') }}
                            </div>
                        </div>
                        <div
                            class="w-8 h-8 rounded-[10px] bg-[#34C759]/10 text-[#34C759] dark:text-[#30D158] flex items-center justify-center shrink-0">
        ...
        <div class="rounded-[14px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 overflow-hidden">
            <div class="px-5 py-4 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <span
                        class="px-2 py-0.5 rounded-full text-[11px] font-medium bg-black/[0.05] dark:bg-white/[0.08] text-black/60 dark:text-white/60 tabular-nums">
            ...
            <div class="hidden sm:block overflow-x-auto">
                <table class="w-full text-left text-[13px] min-w-[620px]">
                    <thead>
                        <tr
                            class="bg-black/[0.02] dark:bg-white/[0.02] border-b border-black/5 dark:border-white/5 text-[11px] font-semibold text-black/50 dark:text-white/50 uppercase tracking-wider">
                        ...
                        @forelse($transactions as $transaction)
                            ...
                            <tr class="hover:bg-black/[0.015] dark:hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="py-3 px-4 whitespace-nowrap">
                                    @if ($isOut && $isTrf)
                                        <span
                                            class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-[#FF3B30]/10 text-[#FF3B30] dark:text-[#FF453A] border border-[#FF3B30]/20">
                                            <span class="w-1.5 h-1.5 rounded-full bg-[#FF3B30] dark:bg-[#FF453A]"></span>
                                            <span>Transfer Keluar</span>
                                        </span>
                                    @elseif(!$isOut && $isTrf)
                                        <span
                                            class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-[#007AFF]/10 text-[#007AFF] dark:text-[#0A84FF] border border-[#007AFF]/20">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-[#34C759]/10 text-[#248A3D] dark:text-[#30D158] border border-[#34C759]/20">
                                ...
                                <td
                                    class="py-3 px-4 text-right tabular-nums font-semibold whitespace-nowrap {{ $isOut ? 'text-[#FF3B30] dark:text-[#FF453A]' : 'text-[#34C759] dark:text-[#30D158]' }}">
                                    {{ $isOut ? '-' : '+' }} Rp {{ number_format($transaction->amount, 0, ',', '.') }}
                                </td>
                                <td
                                    class="py-3 px-4 text-right tabular-nums font-bold text-black dark:text-white whitespace-nowrap">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center">
                                    <div
                                        class="w-12 h-12 rounded-[14px] bg-black/[0.04] dark:bg-white/[0.04] text-black/40 dark:text-white/40 mx-auto flex items-center justify-center mb-3">
                                    ...
                                    <p class="text-[14px] font-semibold text-black dark:text-white">Belum Ada Riwayat
                                        Mutasi</p>
                                    <p class="text-[12px] text-black/50 dark:text-white/50 mt-1">Belum ada catatan mutasi
                                        kas atau bank untuk akun yang dipilih.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                        ...
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($isOut && $isTrf)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-[#FF3B30]/10 text-[#FF3B30] dark:text-[#FF453A]">
                                            Transfer Keluar
                                        </span>
                                    @elseif(!$isOut && $isTrf)
                                        <span
